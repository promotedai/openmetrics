...
public class ContentMetricsJobMiniclusterTest extends BaseJobMiniclusterTest<ContentMetricsJob> {
  ...
  void testSimple() throws Exception {
    ...
    List<LogRequest> inputLogRequests = LogRequestFactory.createLogRequests(
    ...
    MetricsApiKafkaSource.SplitSources splitLogRequest = job.metricsApiKafkaSource.splitSources(fromItems(env, "logRequest", inputLogRequests, LogRequest::getTiming));
    StreamTableEnvironment tableEnv = StreamTableEnvironment.create(env);
    Tables.createCatalogAndDatabase(tableEnv);

    DataStream<JoinedEvent> joinedImpression = fromItems(
            env,
            "joined-impression",
    ...
    TableResult hourlyContentMetrics = tableEnv.executeSql("SELECT * FROM `hourly_content_metrics_view`");

    assertRowsUnorderedEqual(
            ImmutableList.of(Row.of("2022-10-01", "22", 1L, "i-1-1", 1L, 1L, 1L, 5L, 1L, 1L, 2L, 2L, 5000000L)),
            ImmutableList.copyOf(hourlyContentMetrics.collect()));

    // Asserts that some files.  We cannot assert the file contents though because Flink doesn't output all of the rows to
    // the File sink before finishing.
    assertEquals(ImmutableSet.of("hour=22"),
  ...
  void testComplex() throws Exception {
    ...
    List<LogRequest> inputLogRequests = ImmutableList.<LogRequest>builder()
            .addAll(LogRequestFactory.createLogRequests(
                    createLogRequestOptionsBuilder(
                            toEpochMilli(time1),
                            LogRequestFactory.DetailLevel.PARTIAL, 3)
                            .setWriteProductViews(true)
            ...
            .addAll(LogRequestFactory.createLogRequests(
                    createLogRequestOptionsBuilder(
                            ...
                            LogRequestFactory.DetailLevel.PARTIAL, 2)
                            // Use different UUIDs so the events do not get de-duplicated.
                            .setCohortMembershipUuidSupplier(new IncrementingUUIDSupplier("CCCCCCCD-CCCC-CCCC-0000-000000000000"))
                            .setSessionUuidSupplier(new IncrementingUUIDSupplier("11111112-1111-1111-0000-000000000000"))
                            .setViewUuidSupplier(new IncrementingUUIDSupplier("22222223-2222-2222-0000-000000000000"))
                            .setAutoViewUuidSupplier(new IncrementingUUIDSupplier("77777778-7777-7777-0000-000000000000"))
                            .setRequestUuidSupplier(new IncrementingUUIDSupplier("33333334-3333-3333-0000-000000000000"))
                            .setResponseInsertionUuidSupplier(new IncrementingUUIDSupplier("44444445-4444-4444-0000-000000000000"))
                            .setImpressionUuidSupplier(new IncrementingUUIDSupplier("55555556-5555-5555-0000-000000000000"))
                            .setActionUuidSupplier(new IncrementingUUIDSupplier("66666667-6666-6666-0000-000000000000"))
                            .setWriteProductViews(true)
                            .setNavigateCheckoutRate(0.5f)
                            .setNavigateAddToCartRate(0.5f)
                            .setCheckoutPurchaseRate(0.75f)))
            .addAll(LogRequestFactory.createLogRequests(
                    createLogRequestOptionsBuilder(
                            ...
                            LogRequestFactory.DetailLevel.PARTIAL, 1)
                            // Use different UUIDs so the events do not get de-duplicated.
                            .setCohortMembershipUuidSupplier(new IncrementingUUIDSupplier("CCCCCCCE-CCCC-CCCC-0000-000000000000"))
                            .setSessionUuidSupplier(new IncrementingUUIDSupplier("11111113-1111-1111-0000-000000000000"))
                            .setViewUuidSupplier(new IncrementingUUIDSupplier("22222224-2222-2222-0000-000000000000"))
                            .setAutoViewUuidSupplier(new IncrementingUUIDSupplier("77777779-7777-7777-0000-000000000000"))
                            .setRequestUuidSupplier(new IncrementingUUIDSupplier("33333335-3333-3333-0000-000000000000"))
                            .setResponseInsertionUuidSupplier(new IncrementingUUIDSupplier("44444446-4444-4444-0000-000000000000"))
                            .setImpressionUuidSupplier(new IncrementingUUIDSupplier("55555557-5555-5555-0000-000000000000"))
                            .setActionUuidSupplier(new IncrementingUUIDSupplier("66666668-6666-6666-0000-000000000000"))
                            .setWriteProductViews(true)
                            .setNavigateCheckoutRate(0.5f)
                            .setNavigateAddToCartRate(0.5f)
                            .setCheckoutPurchaseRate(0.75f)))
    ...
    TableResult dailyContentMetrics = tableEnv.executeSql("SELECT * FROM `daily_content_metrics_view`");
    assertRowsUnorderedEqual(
            ImmutableList.of(
    ...
    // Asserts that some files.  We cannot assert the file contents though because Flink doesn't output all of the rows to
    // the File sink before finishing.
    assertEquals(ImmutableSet.of("hour=22", "hour=23"),
            filesInDir(new File(tempDir, "blue/etl/hourly_content_metrics/dt=2022-10-01")));
    assertEquals(ImmutableSet.of("hour=00"),
  ...
  void testCsv() throws Exception {
    ...
    // Asserts that some files.  We cannot assert the file contents though because Flink doesn't output all of the rows to
    // the File sink before finishing.
    File[] filesArray = new File(tempDir, "blue/etl/daily_content_metrics_csv/dt=2022-10-01/").listFiles();
    List<File> files = Arrays.stream(filesArray).filter(Predicate.not(ContentMetricsJobMiniclusterTest::isSuccessFile)).collect(Collectors.toList());
    assertEquals(1, files.size());
    String filePath = files.get(0).getPath();
    assertTrue(filePath.endsWith(".csv"), () -> "File should end with a `.csv` suffix, " + filePath);

    List<String> actualLines = Files.readAllLines(Paths.get(files.get(0).getPath()));
    assertEquals(3, actualLines.size());
    assertEquals("date,platform_id,content_id,view_count,impression_count,joined_impression_count,\"joined_impression_position_sum\",navigate_count,add_to_cart_count,checkout_count,purchase_count,gmv_usd_micros",
            actualLines.get(0));
    assertEquals("2022-10-01,1,i-1-1,2,16,1,5,2,2,4,4,10000000",
            actualLines.get(1));
    assertEquals("2022-10-01,1,i-1-10,0,16,0,0,0,0,0,0,0",
            actualLines.get(2));
  ...
  void testCumulatedJSON() throws Exception {
    ...
    DataStream<JoinedEvent> joinedImpression = fromItems(
            ...
            JoinedEvent::getTiming);
    waitForDone(job.createOperators(tableEnv, splitLogRequest, joinedImpression));
    TableResult cumulatedContentMetrics = tableEnv.executeSql("SELECT * FROM `cumulated_content_metrics_view`");
    ...
    File[] filesArray = new File(tempDir, "blue/etl/cumulated_content_metrics/dt=2022-10-01").listFiles();
    List<File> files = Arrays.stream(filesArray).filter(Predicate.not(ContentMetricsJobMiniclusterTest::isSuccessFile)).collect(Collectors.toList());
    assertEquals(1, files.size());
    List<String> actualLines = Files.readAllLines(Paths.get(files.get(0).getPath()));
  ...
  private void waitForDone(Iterable<TableResult> tableResults) throws InterruptedException, ExecutionException {
    for (TableResult tableResult : tableResults) {
      waitForDone(tableResult);
    }
  }

  private void waitForDone(TableResult tableResult) throws InterruptedException, ExecutionException {
  ...
  private static void assertRowsUnorderedEqual(List<Row> expected, List<Row> actual) {
    ...
    assertAll("record",
            Streams.concat(
                    expectedDiffs.stream().map((row) -> () -> {
                      fail("Did not find expected row=" + row);
                    }),
                    actualDiffs.stream().map((row) -> () -> {
  ...
  private static JoinedEvent createTestJoinedImpression(String impressionId, LocalDateTime time, String contentId, long position) {
