...
public class ContentDBFactoryTest {
    ...
    public void onePerType() {
        ContentDB db = ContentDBFactory.create(1L, ContentDBFactoryOptions.builder()
            .setStores(1)
            .setItemsPerStore(1)
        ...
        List<Content> stores = db.listContent(ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.STORE.name()));
        assertEquals(1, stores.size());
        assertEquals(Content.builder()
                .setContentId("s-1-1")
                .setRequestFields(ImmutableMap.of("type", "STORE", "storeId", "s-1-1"))
                .setDocumentFields(ImmutableMap.of("storeName", "Store s-1-1"))
                .build(),
                stores.get(0));
        List<Content> items = db.listContent(ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.ITEM.name()));
        assertEquals(1, items.size());
        assertEquals(Content.builder()
                .setContentId("i-1-1")
                .setRequestFields(ImmutableMap.of("type", "ITEM", "storeId", "s-1-1", "itemId", "i-1-1"))
                .setDocumentFields(ImmutableMap.of("contentName", "Item i-1-1", "storeName", "Store s-1-1"))
                .build(),
                items.get(0));
        List<Content> promotions = db.listContent(ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.PROMOTION.name()));
        assertEquals(1, promotions.size());
        assertEquals(Content.builder()
                        .setContentId("p-1-1")
                        .setRequestFields(ImmutableMap.of(
                                "type", "PROMOTION",
                                "storeId", "s-1-1",
                                "itemId", "i-1-1",
                                "promotionId", "p-1-1"))
    ...
    public void listContent_filters() {
        ...
        assertEquals(1, db.listContent(ImmutableList.of(
                ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.STORE.name()),
                ContentDBFilter.whereFieldIs(ContentType.STORE.id, "s-1-2"))).size());
        ...
        assertEquals(1 + 2 + 4, db.listContent(ContentDBFilter.whereFieldIs(ContentType.STORE.id, "s-1-2")).size());

        assertEquals(1, db.listContent(ImmutableList.of(
                ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.ITEM.name()),
                ContentDBFilter.whereFieldIs(ContentType.ITEM.id, "i-1-4"))).size());
        ...
        assertEquals(1 + 2, db.listContent(ContentDBFilter.whereFieldIs(ContentType.ITEM.id, "i-1-4")).size());

        assertEquals(1, db.listContent(ImmutableList.of(
                ContentDBFilter.whereFieldIs(Content.TYPE, ContentType.PROMOTION.name()),
                ContentDBFilter.whereFieldIs(ContentType.PROMOTION.id, "p-1-8"))).size());
        assertEquals(1, db.listContent(ContentDBFilter.whereContentIdIs("p-1-8")).size());
        // Just finds the one Promotion.
        assertEquals(1, db.listContent(ContentDBFilter.whereFieldIs(ContentType.PROMOTION.id, "p-1-8")).size());
    }
