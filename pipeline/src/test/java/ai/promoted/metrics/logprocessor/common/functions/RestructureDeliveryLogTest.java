...
public class RestructureDeliveryLogTest {
    ...
    public void reservedHeaders() {
        var expectedRequest = Request.newBuilder()
                .addInsertion(Insertion.newBuilder()
                        .setContentId("a")
                        .setRetrievalRank(1)
                        .setRetrievalScore(2.0f)
                );
        var actualRequest = Request.newBuilder()
                .addAllInsertionMatrixHeaders(Arrays.asList("contentId", "retrievalRank", "retrievalScore"))
        ...
        assertEquals(
                ...
                new RestructureDeliveryLog().map(DeliveryLog.newBuilder().setRequest(actualRequest).build()));
    }
    ...
    public void conflictingHeaders() {
        var expectedRequest = Request.newBuilder()
                .addInsertion(Insertion.newBuilder()
                        .setProperties(Properties.newBuilder().setStruct(Struct.newBuilder()
                                .putFields("1", Value.newBuilder().setStructValue(Struct.newBuilder()
                                        .putFields("3", Value.newBuilder().setStringValue("c").build())).build()))));
        var actualRequest = Request.newBuilder()
        ...
        var expectedRequest = Request.newBuilder()
                .addInsertion(Insertion.newBuilder()
                        .setProperties(Properties.newBuilder().setStruct(Struct.newBuilder()
                                ...
                                .putFields("2", Value.newBuilder().setStructValue(Struct.newBuilder()
                                        .putFields("3", Value.newBuilder().setStringValue("b").build())
                                        .putFields("4", Value.newBuilder().setNumberValue(4.0f).build())).build()))));
        var actualRequest = Request.newBuilder()
    ...
    public void multipleInsertions() {
        var expectedRequest = Request.newBuilder()
                ...
                .addInsertion(Insertion.newBuilder()
                        .setProperties(Properties.newBuilder().setStruct(Struct.newBuilder()
                                .putFields("1", Value.newBuilder().setStringValue("b").build()))));
        var actualRequest = Request.newBuilder()
                .addAllInsertionMatrixHeaders(Arrays.asList("1"))
                .setInsertionMatrix(ListValue.newBuilder().addAllValues(Arrays.asList(
                        Value.newBuilder().setListValue(ListValue.newBuilder().addAllValues(Arrays.asList(
                                Value.newBuilder().setStringValue("a").build()
                        ))).build(),
                        Value.newBuilder().setListValue(ListValue.newBuilder().addAllValues(Arrays.asList(
