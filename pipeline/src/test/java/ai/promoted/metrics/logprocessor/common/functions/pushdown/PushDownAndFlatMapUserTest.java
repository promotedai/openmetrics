...
public class PushDownAndFlatMapUserTest extends BasePushDownAndFlatMapTest<User> {
  ...
  @Test
  public void pushAll() {
    new PushDownAndFlatMapUser()
        .flatMap(
            setBaseFields(LogRequest.newBuilder())
                .setUserInfo(
                    UserInfo.newBuilder().setUserId(USER_ID).setLogUserId(LOWERCASE_LOG_USER_ID))
    ...
    assertThat(collector)
        .containsExactly(
            User.newBuilder()
                .setPlatformId(OTHER_PLATFORM_ID)
                .setUserInfo(
                    UserInfo.newBuilder().setUserId(OTHER_USER_ID).setLogUserId(OTHER_LOG_USER_ID))
                .setTiming(createTiming(OTHER_TIME_EPOCH_MILLIS))
                .build());
  }
