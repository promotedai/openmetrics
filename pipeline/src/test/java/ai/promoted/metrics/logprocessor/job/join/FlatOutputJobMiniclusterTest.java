...
public class FlatOutputJobMiniclusterTest extends BaseJobMiniclusterTest<FlatOutputJob> {
  ...
  private Set<String> expectedSideDirs;
  ...
  public void setUp() {
    ...
    expectedEtlDirs = Sets.newHashSet(
            "flat-response-insertion",
            "flat-user-response-insertion",
            "flat-actioned-response-insertion",
            "joined-impression",
            "joined-user-action",
            "joined-action",
            "joined-user-impression");
    expectedSideDirs = Sets.newHashSet("debug");
    expectedDroppedDirs = Sets.newHashSet();
    expectedDebugDirs = Sets.newHashSet(
            "tiny-event",
            "all-delivery-log-request-metadata",
            "all-delivery-log-request",
            "rhs-tiny-action",
            "partial-response-insertion",
            "all-delivery-log-metadata");
  ...
  // Also tests a different job label.  We merged this test case together to have fewer tests (since each test takes a few second).
  ...
  void oneFullEvent(LogRequestFactory.DetailLevel detailLevel, String testName, long timeMillis, String dt, String hour) throws Exception {
    ...
    job.jobLabel = "blue";
    execute(LogRequestFactory.createLogRequestOptionsBuilder(timeMillis, detailLevel, 1)
            .setMiniSdkRate(0.0f)
  ...
  void storeToItem() throws Exception {
    ...
    String actionPath = String.format("etl/joined-action/dt=%s/hour=%s", dt, hour);
    ...
    Map<String, GenericRecord> actionIdToAction =
            actualActions.stream().collect(Collectors.toMap(a -> (String) getNestedField(a, "action", "action_id"), a -> a));
    ...
    JoinedEvent.Builder expectedNavigate = joinedImpressionBuilder.clone().setAction(toExpectedActionField(action.get(0)));
    ...
    expectedAddToCart.getActionBuilder()
            .setContentId("i-1-1")
    ...
    expectedCheckout.getActionBuilder()
            .setContentId("i-1-1")
            ...
            .setSingleCartContent(CartContent.newBuilder()
                    .setContentId("i-1-1")
                    .setQuantity(1)
                    .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(5000000L)))
    ...
    expectedPurchase.getActionBuilder()
            .setContentId("i-1-1")
  ...
  void storeToItem_cart() throws Exception {
    ...
    contentDb = ContentDBFactory.create(LogRequestIterator.PLATFORM_ID, options.contentDBFactoryOptions());
    List<LogRequest> logRequests = ImmutableList.copyOf(new LogRequestIterator(options, contentDb));
    setupJoinInputs(logRequests);

    Map<ActionType, Long> inputActionTypeToCount = action.stream()
            .collect(Collectors.groupingBy(Action::getActionType, Collectors.counting()));
    assertEquals(
            ImmutableMap.of(ActionType.NAVIGATE, 16L, ActionType.CHECKOUT, 4L, ActionType.PURCHASE, 4L, ActionType.ADD_TO_CART, 4L),
    ...
    String impressionPath = String.format("etl/joined-impression/dt=%s/hour=%s", dt, hour);
    ...
    Map<String, Long> actionTypeToCount = actualActions.stream()
    ...
    assertEquals(ImmutableSet.of(
            ...
            "66666666-6666-6666-0000-000000000019"), checkoutIds);

    // CombinedDeliveryLog delays the Request by the window.
    JoinedEvent.Builder joinedImpressionBuilder = createExpectedJoinedImpressionBuilder();
    joinedImpressionBuilder.getIdsBuilder()
    ...
    // Maybe there is an issue with view attribution.
    joinedImpressionBuilder.clearView();
    joinedImpressionBuilder.setRequestInsertion(createExpectedItemRequestInsertion("s-1-1", "i-1-1"));
    joinedImpressionBuilder.getRequestBuilder().setClientRequestId("client-33333333-3333-3333-0000-000000000002");
    ...
    expectedCheckout1.getActionBuilder()
            ...
            .setCart(Cart.newBuilder()
                    ...
                    .addContents(CartContent.newBuilder()
                            ...
                            .setQuantity(4)
                            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(4000000L)))
                    .addContents(CartContent.newBuilder()
                            .setContentId("i-1-201")
                            .setQuantity(3)
                            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(2000000L)))
                    .addContents(CartContent.newBuilder()
                            .setContentId("i-1-221")
                            .setQuantity(1)
                            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(4000000L))));

    Map<String, GenericRecord> contentIdToCheckouts = actualActions.stream()
            .filter(a -> getNestedField(a, "action", "action_id").equals("66666666-6666-6666-0000-00000000001c"))
            .collect(Collectors.toMap(a -> (String) getNestedField(a, "response_insertion", "content_id"), a -> a));
    ...
    expectedCheckout181.getIdsBuilder()
    ...
    expectedCheckout181.getRequestBuilder().setClientRequestId("client-33333333-3333-3333-0000-000000000003");
    expectedCheckout181.setRequestInsertion(createExpectedItemRequestInsertion("s-1-10", "i-1-181"));
    expectedCheckout181.getApiExecutionInsertionBuilder().getFeatureStageBuilder().getFeaturesBuilder().putNumeric(100, 0.5f);
    ...
    expectedCheckout181.getActionBuilder().getSingleCartContentBuilder()
            .setContentId("i-1-181")
            .setQuantity(4)
            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(4000000L));
    ...
    expectedCheckout201.getIdsBuilder()
    ...
    expectedCheckout201.getRequestBuilder().setClientRequestId("client-33333333-3333-3333-0000-000000000004");
    expectedCheckout201.setRequestInsertion(createExpectedItemRequestInsertion("s-1-11", "i-1-201"));
    expectedCheckout201.getApiExecutionInsertionBuilder().getFeatureStageBuilder().getFeaturesBuilder().putNumeric(100, 0.3f);
    ...
    expectedCheckout201.getActionBuilder().getSingleCartContentBuilder()
            .setContentId("i-1-201")
            .setQuantity(3)
            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(2000000L));
    ...
    expectedCheckout221.getIdsBuilder()
    ...
    expectedCheckout221.getRequestBuilder().setClientRequestId("client-33333333-3333-3333-0000-000000000005");
    expectedCheckout221.setRequestInsertion(createExpectedItemRequestInsertion("s-1-12", "i-1-221"));
    ...
    expectedCheckout221.getActionBuilder().getSingleCartContentBuilder()
            .setContentId("i-1-221")
            .setQuantity(1)
            .setPricePerUnit(Money.newBuilder().setCurrencyCode(CurrencyCode.USD).setAmountMicros(4000000L));
    ...
    assertEquals(ImmutableSet.of(
            ...
            "66666666-6666-6666-0000-000000000020"), purchaseIds);

    Map<String, GenericRecord> contentIdToPurchases = actualActions.stream()
            .filter(a -> getNestedField(a, "action", "action_id").equals("66666666-6666-6666-0000-000000000020"))
            .collect(Collectors.toMap(a -> (String) getNestedField(a, "response_insertion", "content_id"), a -> a));
  ...
  void storeToItem_cart_storeInsertion() throws Exception {
    ...
    LogRequestIteratorOptions options = LogRequestFactory.createStoreInsertionItemShoppingCartBuilder().build();
    contentDb = ContentDBFactory.create(LogRequestIterator.PLATFORM_ID, options.contentDBFactoryOptions());
    List<LogRequest> logRequests = ImmutableList.copyOf(new LogRequestIterator(options, contentDb));
    setupJoinInputs(logRequests);
    ...
    Map<ActionType, Long> inputActionTypeToCount = action.stream()
            .collect(Collectors.groupingBy(Action::getActionType, Collectors.counting()));
    assertEquals(
            ImmutableMap.of(ActionType.NAVIGATE, 4L, ActionType.CHECKOUT, 4L, ActionType.PURCHASE, 4L, ActionType.ADD_TO_CART, 4L),
            inputActionTypeToCount);
    ...
    assertEquals(ImmutableSet.of("s-1-1", "s-1-11", "s-1-12", "s-1-10"), contentIdToCheckouts.keySet());
    ...
    // Maybe there is an issue with view attribution.
    joinedImpressionBuilder.clearView();
    joinedImpressionBuilder.getRequestBuilder().setClientRequestId("client-33333333-3333-3333-0000-000000000001");
    ...
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualCheckout10, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000002", getNestedField(actualCheckout10, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000002", getNestedField(actualCheckout10, "ids", "impression_id"));
    ...
    assertEquals("i-1-181", getNestedField(actualCheckout10, "action", "single_cart_content", "content_id"));

    GenericRecord actualCheckout11 = contentIdToCheckouts.get("s-1-11");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualCheckout11, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000003", getNestedField(actualCheckout11, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000003", getNestedField(actualCheckout11, "ids", "impression_id"));
    ...
    assertEquals("i-1-201", getNestedField(actualCheckout11, "action", "single_cart_content", "content_id"));

    GenericRecord actualCheckout12 = contentIdToCheckouts.get("s-1-12");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualCheckout12, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000004", getNestedField(actualCheckout12, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000004", getNestedField(actualCheckout12, "ids", "impression_id"));
    ...
    assertEquals("i-1-221", getNestedField(actualCheckout12, "action", "single_cart_content", "content_id"));
    ...
    Map<String, GenericRecord> contentIdToPurchases = actualActions.stream()
            .filter(a -> getNestedField(a, "action", "action_id").equals("66666666-6666-6666-0000-00000000001d"))
            .collect(Collectors.toMap(a -> (String) getNestedField(a, "response_insertion", "content_id"), a -> a));
    assertEquals(4, contentIdToCheckouts.size());

    assertEquals(ImmutableSet.of("s-1-1", "s-1-11", "s-1-12", "s-1-10"), contentIdToPurchases.keySet());

    GenericRecord actualPurchase1 = contentIdToPurchases.get("s-1-1");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualPurchase1, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000001", getNestedField(actualPurchase1, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000001", getNestedField(actualPurchase1, "ids", "impression_id"));
    ...
    assertEquals("i-1-1", getNestedField(actualPurchase1, "action", "single_cart_content", "content_id"));

    GenericRecord actualPurchase10 = contentIdToPurchases.get("s-1-10");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualPurchase10, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000002", getNestedField(actualPurchase10, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000002", getNestedField(actualPurchase10, "ids", "impression_id"));
    ...
    assertEquals("i-1-181", getNestedField(actualPurchase10, "action", "single_cart_content", "content_id"));

    GenericRecord actualPurchase11 = contentIdToPurchases.get("s-1-11");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualPurchase11, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000003", getNestedField(actualPurchase11, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000003", getNestedField(actualPurchase11, "ids", "impression_id"));
    ...
    assertEquals("i-1-201", getNestedField(actualPurchase11, "action", "single_cart_content", "content_id"));

    GenericRecord actualPurchase12 = contentIdToPurchases.get("s-1-12");
    assertEquals("33333333-3333-3333-0000-000000000001", getNestedField(actualPurchase12, "ids", "request_id"));
    assertEquals("44444444-4444-4444-0000-000000000004", getNestedField(actualPurchase12, "ids", "insertion_id"));
    assertEquals("55555555-5555-5555-0000-000000000004", getNestedField(actualPurchase12, "ids", "impression_id"));
    ...
    assertEquals("i-1-221", getNestedField(actualPurchase12, "action", "single_cart_content", "content_id"));
  }
    ...
    return Insertion.newBuilder()
            .setProperties(Properties.newBuilder()
                    .setStruct(Struct.newBuilder()
                            .putFields("storeId", Value.newBuilder()
                                    .setStringValue(storeId)
                                    .build())
                            .putFields("type", Value.newBuilder()
                                    .setStringValue("STORE")
                                    .build())
                            .build()))
            .build();
  }
  ...
  private Insertion createExpectedItemRequestInsertion(String storeId, String itemId) {
    return Insertion.newBuilder()
            .setProperties(Properties.newBuilder()
                    .setStruct(Struct.newBuilder()
                            .putFields("itemId", Value.newBuilder()
  ...
  void combineSdkAndShadowTrafficDeliveryLogs() throws Exception {
    ...
    JoinedEvent.Builder joinedImpressionBuilder = createExpectedJoinedImpressionBuilder()
            ...
            .setApiExecutionInsertion(toExpectedExecutionInsertionField(deliveryLog.get(1).getExecution().getExecutionInsertion(0)))
            .setSdkExecution(DeliveryExecution.newBuilder()
  ...
  void filterOutIgnoreUsage() throws Exception {
    execute(LogRequestFactory.createLogRequestOptionsBuilder(timeMillis, detailLevel, 1)
            .setMiniSdkRate(0.0f)
            .setShadowTrafficRate(0.0f)
            .setDeliveryLogTransform(builder -> {
    ...
    assertEtlOutputCount();
    expectedSideDirs.add("dropped");
    expectedDroppedDirs = ImmutableSet.of("ignore-usage-joined-action", "ignore-usage-joined-impression");
    assertSideOutputCount();
    assertEquals(null, new File(tempDir, "etl/joined-impression").list());
    assertEquals(null, new File(tempDir, "etl/joined-action").list());
  }
  ...
  void filterOutNonBuyer() throws Exception {
    ...
    execute(LogRequestFactory.createLogRequestOptionsBuilder(timeMillis, detailLevel, 1)
            ...
            .setShadowTrafficRate(0.0f)
            .setDeliveryLogTransform(builder -> {
              builder.getExecutionBuilder().getExecutionInsertionBuilder(0).getFeatureStageBuilder().getFeaturesBuilder().putSparseId(
                      isStaffHash, 1L);
              return builder;
            }));
    ...
    assertEtlOutputCount();
    expectedSideDirs.add("dropped");
    expectedDroppedDirs = ImmutableSet.of("non-buyer-joined-impression", "non-buyer-joined-action");
    assertSideOutputCount();
    assertEquals(null, new File(tempDir, "etl/joined-impression").list());
    assertEquals(null, new File(tempDir, "etl/joined-action").list());
  }
  ...
  void filterOutBot() throws Exception {
    ...
    execute(LogRequestFactory.createLogRequestOptionsBuilder(timeMillis, detailLevel, 1)
            ...
            .setShadowTrafficRate(0.0f)
            .setDeliveryLogTransform(builder -> {
              builder.getRequestBuilder().getDeviceBuilder().getBrowserBuilder().setUserAgent("google bot");
              return builder;
            }));
    ...
    assertEtlOutputCount();
    expectedSideDirs.add("dropped");
    // BUG - Drops ins-imp and imp-act because we filter out DeliveryLogs too early to save resources.
    // We should have a more efficient way to filter these through the system.
    expectedDroppedDirs = ImmutableSet.of("delivery-log-is-bot", "impression-actions", "insertion-impressions");
    expectedDebugDirs = ImmutableSet.of("rhs-tiny-action");
    assertSideOutputCount();
    assertEquals(null, new File(tempDir, "etl/joined-impression").list());
    assertEquals(null, new File(tempDir, "etl/joined-action").list());
  }
  ...
  // Also tests a different job label.  We merged this test case together to have fewer tests (since each test takes a few second).
  @DisplayName("One full event > no flat_action_response_insertion when actions are missing")
  @Test
  void noFlatActionedResponseInsertionsWhenActionsAreMissing() throws Exception {

    job.jobLabel = "blue";
    execute(LogRequestFactory.createLogRequestOptionsBuilder(timeMillis, detailLevel, 1)
            .setImpressionNavigateRate(0.0f)
            .setMiniSdkRate(0.0f)
            .setShadowTrafficRate(0.0f));
    ...
    // Just assert that the file isn't written.
    expectedEtlDirs.remove("flat-actioned-response-insertion");
    expectedEtlDirs.remove("joined-action");
    expectedEtlDirs.remove("joined-user-action");
    assertEtlOutputCount();
    expectedDebugDirs.remove("rhs-tiny-action");
    assertSideOutputCount();
  }
  ...
  private void assertJoinedImpression(JoinedEvent joinedImpression) throws IOException {
    assertRecordInFile("etl/joined-impression/dt=%s/hour=%s", joinedImpression, "etl/joined-impression");
  }

  private void assertJoinedUserImpression(JoinedEvent.Builder builder) throws IOException {
    assertJoinedUserImpression(builder.build());
  }

  private void assertJoinedUserImpression(JoinedEvent joinedUserImpression) throws IOException {
    assertRecordInFile("etl/joined-user-impression/dt=%s/hour=%s", joinedUserImpression, "etl/joined-user-impression");
  }
  ...
  private void assertJoinedAction(JoinedEvent joinedAction) throws IOException {
    assertRecordInFile("etl/joined-action/dt=%s/hour=%s", joinedAction, "etl/joined-action");
  }

  private void assertJoinedUserAction(JoinedEvent.Builder builder) throws IOException {
    assertJoinedUserAction(builder.build());
  }

  private void assertJoinedUserAction(JoinedEvent joinedUserAction) throws IOException {
    assertRecordInFile("etl/joined-user-action/dt=%s/hour=%s", joinedUserAction, "etl/joined-user-action");
  }

  private void assertFlatResponseInsertion(FlatResponseInsertion.Builder builder) throws IOException {
    assertFlatResponseInsertion(builder.build());
  }

  private void assertFlatResponseInsertion(FlatResponseInsertion flatResponseInsertion) throws IOException {
    assertRecordInFile("etl/flat-response-insertion/dt=%s/hour=%s", flatResponseInsertion, "etl/flat-response-insertion");
  }

  private void assertFlatUserResponseInsertion(FlatResponseInsertion.Builder builder) throws IOException {
    assertFlatUserResponseInsertion(builder.build());
  }

  private void assertFlatUserResponseInsertion(FlatResponseInsertion flatResponseInsertion) throws IOException {
    assertRecordInFile("etl/flat-user-response-insertion/dt=%s/hour=%s", flatResponseInsertion, "etl/flat-user-response-insertion");
  }

  private void assertRecordInFile(String template, GeneratedMessageV3 record, String message) throws IOException {
  ...
  private void assertEtlOutputCount() {
    assertEquals(expectedEtlDirs, ImmutableSet.copyOf(nullToEmptyArray(new File(tempDir, "etl").list())));
  }

  private void assertSideOutputCount() {
    assertEquals(expectedSideDirs, ImmutableSet.copyOf(nullToEmptyArray(new File(tempDir, "etl-side").list())));
    assertEquals(0, nullToEmptyArray(new File(tempDir, "etl-side/invalid").list()).length);
    assertEquals(expectedLateDirs, ImmutableSet.copyOf(nullToEmptyArray(new File(tempDir, "etl-side/late").list())));
    assertEquals(expectedDroppedDirs, ImmutableSet.copyOf(nullToEmptyArray(new File(tempDir, "etl-side/dropped").list())));
    assertEquals(expectedDebugDirs, ImmutableSet.copyOf(nullToEmptyArray(new File(tempDir, "etl-side/debug").list())));
  }
  ...
  private JoinedEvent.Builder createExpectedJoinedImpressionBuilder() {
    long logTimestamp = deliveryLog.get(0).getRequest().getTiming().getEventApiTimestamp();
    JoinedEvent.Builder joinedImpressionBuilder = JoinedEvent.newBuilder()
            ...
            .setRequestInsertion(toExpectedRequestInsertionField(deliveryLog.get(0).getRequest().getInsertion(0)))
            .setResponse(toExpectedResponseField(Response.getDefaultInstance()))
            .setResponseInsertion(toExpectedResponseInsertionField(deliveryLog.get(0).getResponse().getInsertion(0)))
            .setApiExecution(DeliveryExecution.newBuilder()
                    .setExecutionServer(ExecutionServer.API)
                    .build())
    ...
    if (deliveryLog.get(0).getExecution().getExecutionInsertionCount() > 0) {
      joinedImpressionBuilder.setApiExecutionInsertion(toExpectedExecutionInsertionField(deliveryLog.get(0).getExecution().getExecutionInsertion(0)));
    }
  ...
  private void assertJoinedOneFullImpressionAndAction(JoinedEvent.Builder expectedJoinedImpressionBuilder, long logTimestamp) throws IOException {
    assertEtlOutputCount();
    assertJoinedImpression(expectedJoinedImpressionBuilder.clone());
    assertJoinedUserImpression(setExpectedUser(expectedJoinedImpressionBuilder.clone(), logTimestamp));
    JoinedEvent.Builder joinedActionBuilder = expectedJoinedImpressionBuilder.clone().setAction(toExpectedActionField(action.get(0)));
    assertJoinedAction(joinedActionBuilder.clone());
    assertJoinedUserAction(setExpectedUser(joinedActionBuilder, logTimestamp));
    ...
    try {
      ...
      assertFlatUserResponseInsertion(setExpectedUser(flatResponseInsertion, logTimestamp));
    } catch (Throwable e) {
      // Track down a flake.
      if (true) {
        throw new RuntimeException("Adding extra info.  flatResponseInsertion=" + flatResponseInsertion.clone().build(), e);
      }
    }
  }
  ...
  private FlatResponseInsertion.Builder setExpectedUser(FlatResponseInsertion.Builder builder, long logTimestamp) {
    User expectedFlatUser = toExpectedUserField(user.get(0), logTimestamp);
    return builder
            .setIds(builder.getIdsBuilder().clone().setUserId("userId1"))
            .setUser(expectedFlatUser);
  ...
  // Separate function to reduce test lines of code.  The JoinedIdentifiers code clears a bunch of these fields.
  ...
  private Request toExpectedRequestField(Request request, long logTimestamp) {
    Request.Builder builder = request.toBuilder()
      .clearPlatformId()
      .clearUserInfo()
      .clearRequestId()
      .clearViewId()
  ...
  private Insertion toExpectedRequestInsertionField(Insertion insertion) {
    return toBaseExpectedInsertionField(insertion).toBuilder()
            .clearContentId()
            .build();
  }
  ...
  private Insertion toBaseExpectedInsertionField(Insertion insertion) {
    Insertion.Builder builder = insertion.toBuilder()
            .clearPlatformId()
            .clearUserInfo()
            .clearTiming()
            .clearSessionId()
  ...
  public static class MockContentFieldLookup implements ContentAPILoader {
    ...
    private Map<String, Object> getSingleContentFields(String contentId) {
      ...
      Map<String, Object> otherContentIds = content.requestFields().entrySet().stream()
              // Do not include the contentId field again.
              .filter(entry -> matchingMockKey(entry.getKey()) && !content.contentId().equals(entry.getValue()))
    ...
    private boolean matchingMockKey(String key) {
      return key.equals(ContentType.ITEM.id) || key.equals(ContentType.STORE.id) || key.equals(ContentType.PROMOTION.id);
    }
  }
  ...
  private void executeJoinEvents(
      ...
      List<Action> actions,
      ContentDB contentDb) throws Exception {
    ...
    job.executeJoinEvents(
            fromCollectionSource(
                    env,
                    "user",
                    new CollectionSource<>(users, e -> e.getTiming().getEventApiTimestamp()),
            ...
            fromCollectionSource(
                    env,
                    "delivery-log",
                    new CollectionSource<>(deliveryLogs, d -> d.getRequest().getTiming().getEventApiTimestamp()),
