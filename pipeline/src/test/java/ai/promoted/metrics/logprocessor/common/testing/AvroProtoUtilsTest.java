 ...
 *
 * http://apache-flink-user-mailing-list-archive.2336050.n4.nabble.com/Minicluster-Flink-tests-checkpoints-and-inprogress-part-files-td41282.html
 */
public class AvroProtoUtilsTest {
  ...
  List<Impression> getTestProtos() {
    return LogRequestFactory.pushDownToImpressions(LogRequestFactory.createFullLogRequests(1601596151000L, 1));
  }
            ...
            .setRequest(Request.newBuilder()
                    .setProperties(Properties.newBuilder()
                            .setStruct(Struct.newBuilder()
                                    .putFields("stringKey", Value.newBuilder().setStringValue("a").build())
                                    .putFields("structKey", Value.newBuilder().setStructValue(Struct.newBuilder()
                                            .putFields("stringKey2", Value.newBuilder().setStringValue("b").build())
                                            .putFields("structKey2", Value.newBuilder().setStructValue(Struct.newBuilder()
                                                    .putFields("stringKey3", Value.newBuilder().setStringValue("b").build()))
                                                    .build()))
                                            .build()))))
            .build();
