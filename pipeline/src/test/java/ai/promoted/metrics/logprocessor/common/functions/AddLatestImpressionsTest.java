...
public class AddLatestImpressionsTest {
    ...
    private static JoinedEvent createFlatAction(String impressionId, String contentId, long clientLogTimestamp) {
        return JoinedEvent.newBuilder()
                ...
                .setAction(Action.newBuilder()
                        .setTiming(Timing.newBuilder()
                                ...
                                .setEventApiTimestamp(clientLogTimestamp + EVENT_API_TIMESTAMP_DELAY.toMillis())))
                .build();
    }

    private static JoinedEvent addFlatImpressions(JoinedEvent joinedEvent, List<LatestImpression> latestImpressions) {
        return joinedEvent.toBuilder()
                .setLatestImpressions(LatestImpressions.newBuilder()
                        .addAllImpression(latestImpressions))
                .build();
    }

    private JoinedEvent createFlatImpression(String impressionId, String contentId, long clientLogTimestamp) {
        return JoinedEvent.newBuilder()
                .setIds(JoinedIdentifiers.newBuilder().setImpressionId(impressionId))
                .setResponseInsertion(Insertion.newBuilder()
                        .setContentId(contentId))
    ...
    private KeyedTwoInputStreamOperatorTestHarness createHarness(AddLatestImpressions addLatestImpressions) throws Exception {
        KeyedTwoInputStreamOperatorTestHarness harness = ProcessFunctionTestHarnesses.forKeyedCoProcessFunction(
                addLatestImpressions,
                joinedEvent -> Tuple2.of(joinedEvent.getIds().getPlatformId(), joinedEvent.getIds().getLogUserId()),
                joinedEvent -> Tuple2.of(joinedEvent.getIds().getPlatformId(), joinedEvent.getIds().getLogUserId()),
        ...
        harness.getExecutionConfig().registerTypeWithKryoSerializer(
                LatestImpression.class, ProtobufSerializer.class);
        harness.getExecutionConfig().registerTypeWithKryoSerializer(
     ...
     */
    private static void processElement1(KeyedTwoInputStreamOperatorTestHarness harness, JoinedEvent action) throws Exception {
        harness.processElement1(action, action.getAction().getTiming().getClientLogTimestamp() + KAFKA_TIMESTAMP_DELAY.toMillis());
    }

    /**
     * Simplifies specifying a kafka delay to the event time.
     */
    private static void processElement2(KeyedTwoInputStreamOperatorTestHarness harness, JoinedEvent impression) throws Exception {
        harness.processElement2(impression, impression.getImpression().getTiming().getClientLogTimestamp() + KAFKA_TIMESTAMP_DELAY.toMillis());
    }
    ...
    public void noEvents() throws Exception {
        AddLatestImpressions addLatestImpressions = new AddLatestImpressions(Duration.ofSeconds(1), true);
        ...
        harness.processBothWatermarks(new Watermark(TimeUnit.HOURS.toMillis(Constants.SESSION_DURATION_HOURS + 1)));
        assertEquals(0, harness.numEventTimeTimers());
    }

    @Test
    public void oneImpression() throws Exception {
        AddLatestImpressions addLatestImpressions = new AddLatestImpressions(
    ...
    public void multipleImpressionsVeryQuick() throws Exception {
        AddLatestImpressions addLatestImpressions = new AddLatestImpressions(Duration.ofSeconds(0), true);
        KeyedTwoInputStreamOperatorTestHarness harness = createHarness(addLatestImpressions);
    ...
    public void multipleImpressionsSessionClears() throws Exception {
        ...
        harness.processBothWatermarks(new Watermark(timestamp3 + TimeUnit.HOURS.toMillis(Constants.SESSION_DURATION_HOURS + 1)));
        assertEquals(0, harness.numEventTimeTimers());
        assertSize(0, addLatestImpressions.actions);
        assertSize(0, addLatestImpressions.impressions);
        assertSize(0, addLatestImpressions.actionedImpressions);
    ...
    public void oneAction() throws Exception {
        ...
        assertEquals(ImmutableList.of(
                addFlatImpressions(
                        createFlatAction("123", "itemABC", 1000),
                        ImmutableList.of())),
                harness.extractOutputValues());
    ...
    public void multipleActionsSessionClears() throws Exception {
        ...
        harness.processBothWatermarks(new Watermark(timestamp3 + 1001 + KAFKA_TIMESTAMP_DELAY.toMillis()));
        ...
        assertEquals(ImmutableList.of(
                addFlatImpressions(
                        ...
                        ImmutableList.of()),
                addFlatImpressions(
                        createFlatAction("234", "itemBCD", timestamp2),
                        ImmutableList.of()),
                addFlatImpressions(
                        createFlatAction("345", "itemCDE", timestamp3),
                        ImmutableList.of())),
                harness.extractOutputValues());
        assertEquals(1, harness.numEventTimeTimers());
        assertSize(0, addLatestImpressions.actions);
    ...
    private void assertOneImpressionAndOneAction(Duration maxOutOfOrderness) throws Exception {
        ...
        harness.processBothWatermarks(new Watermark(999 + + KAFKA_TIMESTAMP_DELAY.toMillis() +  maxOutOfOrderness.toMillis()));
        assertEquals(ImmutableList.of(), harness.extractOutputValues());

        harness.processBothWatermarks(new Watermark(1000 + KAFKA_TIMESTAMP_DELAY.toMillis() + maxOutOfOrderness.toMillis()));
        assertEquals(ImmutableList.of(
                addFlatImpressions(
                        createFlatAction("123", "itemABC", 1000),
                        ImmutableList.of())),
    ...
    private void assertMultipleImpressionsAndOneAction(Duration maxOutOfOrderness) throws Exception {
        ...
        harness.processBothWatermarks(new Watermark(9999 + KAFKA_TIMESTAMP_DELAY.toMillis() + maxOutOfOrderness.toMillis()));
        assertEquals(ImmutableList.of(), harness.extractOutputValues());
        harness.processBothWatermarks(new Watermark(10000 + KAFKA_TIMESTAMP_DELAY.toMillis() + maxOutOfOrderness.toMillis()));
        assertEquals(
                ImmutableList.of(addFlatImpressions(
    ...
    public void multipleImpressionsAndActions() throws Exception {
        // Having maxStateImpressions be higher is important for this test.  Otherwise we drop itemF in the last group.
        AddLatestImpressions addLatestImpressions = new AddLatestImpressions(Duration.ofSeconds(1), 3, 5, true);
        KeyedTwoInputStreamOperatorTestHarness harness = createHarness(addLatestImpressions);
        ...
        assertEquals(
                ImmutableList.of(
                        addFlatImpressions(
                                createFlatAction("3", "itemC", 10000),
                                ImmutableList.of(
                                        // We don't want to include itemC because it was actioned on and this list is to give
    ...
    public void getActionTimestamp() throws Exception {
        ...
        assertEquals(456, AddLatestImpressions.getActionTimestamp(
                JoinedEvent.newBuilder()
                        .setAction(Action.newBuilder().setTiming(Timing.newBuilder()
                                .setEventApiTimestamp(456)))
                        .build()));
        assertEquals(123, AddLatestImpressions.getActionTimestamp(
                JoinedEvent.newBuilder()
                        .setAction(Action.newBuilder().setTiming(Timing.newBuilder()
    ...
    public void getImpressionTimestamp() throws Exception {
        assertEquals(123, AddLatestImpressions.getImpressionTimestamp(createFlatImpression("", "", 123)));
        assertEquals(456, AddLatestImpressions.getImpressionTimestamp(
                JoinedEvent.newBuilder()
                        .setImpression(Impression.newBuilder().setTiming(Timing.newBuilder()
                                .setEventApiTimestamp(456)))
                        .build()));
        assertEquals(123, AddLatestImpressions.getImpressionTimestamp(
                JoinedEvent.newBuilder()
                        .setImpression(Impression.newBuilder().setTiming(Timing.newBuilder()
                                .setClientLogTimestamp(123)
                                .setEventApiTimestamp(456)))
