...
public class CombineDeliveryLogTest {
    private static DeliveryLog createDeliveryLog(String requestId, String clientRequestId, ExecutionServer executionServer, ClientInfo.TrafficType trafficType, String pagingId) {
    ...
    private static DeliveryLog createDeliveryLog(String requestId, String clientRequestId, ExecutionServer executionServer, ClientInfo.TrafficType trafficType) {
        return createDeliveryLog(requestId, clientRequestId, executionServer, trafficType, "");
    }

    private static DeliveryLog createDeliveryLog(String requestId, String clientRequestId, ExecutionServer executionServer, String pagingId) {
        return createDeliveryLog(requestId, clientRequestId, executionServer, ClientInfo.TrafficType.PRODUCTION, pagingId);
    }

    private static DeliveryLog createDeliveryLog(String requestId, String clientRequestId, ExecutionServer executionServer) {
        return createDeliveryLog(requestId, clientRequestId, executionServer, ClientInfo.TrafficType.PRODUCTION);
    }

    private KeyedOneInputStreamOperatorTestHarness<Tuple2<String, Long>, DeliveryLog, CombinedDeliveryLog>
            createHarness(CombineDeliveryLog combineDeliveryLog) throws Exception {
        KeyedOneInputStreamOperatorTestHarness harness = ProcessFunctionTestHarnesses.forKeyedProcessFunction(
                combineDeliveryLog,
        ...
        harness.getExecutionConfig().registerTypeWithKryoSerializer(
                DeliveryLog.class, ProtobufSerializer.class);
        return harness;
    }

    @Test
    public void noEvents() throws Exception {
        CombineDeliveryLog combineDeliveryLog = new CombineDeliveryLog(Duration.ofMillis(5000L), DebugIds.empty());
    ...
    public void oneApi() throws Exception {
        CombineDeliveryLog combineDeliveryLog = new CombineDeliveryLog(Duration.ofMillis(5000L), DebugIds.empty());
        KeyedOneInputStreamOperatorTestHarness harness = createHarness(combineDeliveryLog);
        harness.processElement(createDeliveryLog("req1", "clientReq1", ExecutionServer.API, "pagingId1"), 100);
        assertEquals(1, harness.numEventTimeTimers());
        harness.processWatermark(new Watermark(10000));
        assertEquals(ImmutableList.of(
                CombinedDeliveryLog.newBuilder()
    ...
    public void lowerPriDoesNotGetSet() throws Exception {
        ...
        KeyedOneInputStreamOperatorTestHarness harness = createHarness(combineDeliveryLog);
        harness.processElement(createDeliveryLog("req1", "clientReq1", ExecutionServer.API), 100);
        harness.processElement(createDeliveryLog("req2", "clientReq1", ExecutionServer.API, ClientInfo.TrafficType.SHADOW), 200);
        harness.processElement(createDeliveryLog("req3", "clientReq1", ExecutionServer.SDK), 600);
        harness.processElement(createDeliveryLog("req4", "clientReq1", ExecutionServer.SDK, ClientInfo.TrafficType.REPLAY), 700);
        assertEquals(1, harness.numEventTimeTimers());
        harness.processWatermark(new Watermark(10000));
        assertEquals(ImmutableList.of(
                CombinedDeliveryLog.newBuilder()
    ...
    public void canReuseClientRequestIdAfterWindow() throws Exception {
        ...
        assertEquals(1, harness.numEventTimeTimers());
        harness.processWatermark(new Watermark(10000));
        CombinedDeliveryLog expectedCombinedDeliveryLog = CombinedDeliveryLog.newBuilder()
                .setApi(createDeliveryLog("req1", "clientReq1", ExecutionServer.API))
                .setSdk(createDeliveryLog("req2", "clientReq1", ExecutionServer.SDK))
                .build();
        ...
        assertEquals(ImmutableList.of(expectedCombinedDeliveryLog, expectedCombinedDeliveryLog), harness.extractOutputValues());
        assertEquals(0, harness.numEventTimeTimers());
    }
