...
public class SlidingHourlyCounterTest {
    ...
    public void testCounting() throws Exception {
        SlidingHourlyCounter<String, JoinedEvent> function =
                new SlidingHourlyCounter<>(
                        Types.STRING, Duration.ofMinutes(15), FlatUtil::getEventApiTimestamp, CounterUtil::getCount, true);
        ...
        harness.processElement(
                joinedEvent(createAction(ActionType.NAVIGATE, 21)),
        ...
        assertEquals(3, harness.numKeyedStateEntries());
        assertEquals(4, harness.numEventTimeTimers());
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("UNKNOWN_ACTION_TYPE", 1, 1L, 0), Duration.ofMinutes(15).toMillis()),
                new StreamRecord<>(Tuple4.of("NAVIGATE", 1, 2L, 0), Duration.ofMinutes(15).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
        ...
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("15m border", 1, 1L, 0), Duration.ofMinutes(30).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
        ...
        assertEquals(4, harness.numEventTimeTimers());
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("NAVIGATE", 1, 3L, 0), Duration.ofMinutes(45).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
        ...
        assertEquals(3, harness.numEventTimeTimers());
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("NAVIGATE", 1, 1L, 0), Duration.ofMinutes(75).toMillis()),
                new StreamRecord<>(Tuple4.of("UNKNOWN_ACTION_TYPE", 1, 0L, 0), Duration.ofMinutes(75).toMillis()),
                new StreamRecord<>(Tuple4.of("15m border", 1, 0L, 0), Duration.ofMinutes(90).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
        ...
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("NAVIGATE", 1, 0L, 0), Duration.ofMinutes(105).toMillis()),
                new StreamRecord<>(Tuple4.of("LIKE", 1, 1L, 0), Duration.ofMinutes(105).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
        ...
        expected = expected.append(
                new StreamRecord<>(Tuple4.of("LIKE", 1, 0L, 0), Duration.ofMinutes(165).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
    }
    ...
    public void testShoppingCartCount() throws Exception {
        SlidingHourlyCounter<String, JoinedEvent> function =
                new SlidingHourlyCounter<>(
                        Types.STRING, Duration.ofMinutes(15), FlatUtil::getEventApiTimestamp, CounterUtil::getCount, false);
        KeyedOneInputStreamOperatorTestHarness<String, JoinedEvent, Tuple4<String, Integer, Long, Integer>> harness =
                new KeyedOneInputStreamOperatorTestHarness<>(
                        new KeyedProcessOperator<>(function),
                        FlatUtil::getActionString,
        ...
        assertEquals(1, harness.numEventTimeTimers());
        FluentIterable<StreamRecord<Tuple4<String, Integer, Long, Integer>>> expected = FluentIterable.of();
        expected = expected.append(new StreamRecord<>(Tuple4.of("PURCHASE", 1, 5L, 0), Duration.ofMinutes(15).toMillis()));
        assertIterableEquals(expected, harness.extractOutputStreamRecords());
    }
    ...
    private static SlidingHourlyCounter<Void, JoinedEvent> createSlidingHourlyCounter(Duration emitWindow) {
        return new SlidingHourlyCounter<>(Types.VOID, emitWindow, FlatUtil::getEventApiTimestamp, CounterUtil::getCount, false);
    }

    @Test
    public void testEmitWindow() {
        // Factors are fine.
        assertThrows(IllegalArgumentException.class, () -> createSlidingHourlyCounter(Duration.ofMinutes(0)));
        ...
        assertThrows(IllegalArgumentException.class, () -> createSlidingHourlyCounter(Duration.ofMinutes(7)));
        assertThrows(IllegalArgumentException.class, () -> createSlidingHourlyCounter(Duration.ofMinutes(8)));
        ...
        assertThrows(IllegalArgumentException.class, () -> createSlidingHourlyCounter(Duration.ofMinutes(31)));
        createSlidingHourlyCounter(Duration.ofMinutes(60));
        assertThrows(IllegalArgumentException.class, () -> createSlidingHourlyCounter(Duration.ofMinutes(120)));
    }
