...
public class CachingContentDataStreamLookupTest {
    ...
    @BeforeEach
    public void setUp() {
        lookup = new CachingContentDataStreamLookup(100, Duration.ofSeconds(10),
                (contentIds) -> {
    ...
    public void asyncInvoke() {
        ...
        lookup.asyncInvoke(event, future);
        verify(future).complete(Collections.singleton(
                event.toBuilder().putOtherContentIds(StringUtil.hash("parent1"), "value1").build()));
    }
}
