...
public class SetLogTimestampTest {
  ...
  public void processElement() throws Exception {
    Mockito.when(mockContext.timestamp()).thenReturn(1000L);
    Action action = Action.getDefaultInstance();
    new SetLogTimestamp<Action, Action.Builder>(
    ...
    Mockito.verify(mockCollector)
        .collect(action.toBuilder().setTiming(Timing.newBuilder().setLogTimestamp(1000L)).build());
  }
}
