...
public class PromotedRowDataToCsvConverters implements Serializable {
   ...
   */
  interface PromotedRowDataToCsvConverter
      extends Converter<
          RowData, JsonNode, PromotedRowDataToCsvConverter.RowDataToCsvFormatConverterContext> {
    /**
  ...
  public static PromotedRowDataToCsvConverter createRowConverter(RowType type) {
    ...
    final RowFieldConverter[] fieldConverters =
        Arrays.stream(fieldTypes)
            .map(PromotedRowDataToCsvConverters::createNullableRowFieldConverter)
    ...
    return (row, context) -> {
      ...
      for (int i = 0; i < rowArity; i++) {
        try {
          objectNode.set(
              fieldNames[i],
              fieldConverters[i].convert(context.csvMapper, context.container, row, i));
        } catch (Throwable t) {
  ...
  private static RowFieldConverter createRowFieldConverter(LogicalType fieldType) {
    switch (fieldType.getTypeRoot()) {
      ...
      case TIMESTAMP_WITHOUT_TIME_ZONE:
        ...
        return (csvMapper, container, row, pos) ->
            convertTimestamp(
                row.getTimestamp(pos, timestampPrecision), container, SQL_TIMESTAMP_FORMAT);
  ...
  private static ArrayElementConverter createArrayElementConverter(LogicalType fieldType) {
    switch (fieldType.getTypeRoot()) {
      ...
      case TIMESTAMP_WITHOUT_TIME_ZONE:
        ...
        return (csvMapper, container, array, pos) ->
            convertTimestamp(
                array.getTimestamp(pos, timestampPrecision), container, SQL_TIMESTAMP_FORMAT);
      case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
