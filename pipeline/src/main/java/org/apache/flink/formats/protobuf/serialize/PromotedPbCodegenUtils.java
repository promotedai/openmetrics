...
public class PromotedPbCodegenUtils {
  private static final Logger LOG = LogManager.getLogger(PromotedPbCodegenUtils.class);
    ...
    switch (fd.getJavaType()) {
      case MESSAGE:
        if (fd.isMapField()) {
          ...
          // simple message
          typeStr = PromotedPbFormatUtils.getFullJavaName(fd.getMessageType());
        }
        break;
      case INT:
        typeStr = "Integer";
      ...
      case STRING:
        ...
        break;
      case ENUM:
        typeStr = PromotedPbFormatUtils.getFullJavaName(fd.getEnumType());
        break;
      case FLOAT:
        typeStr = "Float";
        break;
  ...
  public static String pbDefaultValueCode(
      FieldDescriptor fieldDescriptor, PbFormatContext pbFormatContext) throws PbCodegenException {
    String nullLiteral = pbFormatContext.getPbFormatConfig().getWriteNullStringLiterals();
    switch (fieldDescriptor.getJavaType()) {
      case MESSAGE:
        return PromotedPbFormatUtils.getFullJavaName(fieldDescriptor.getMessageType())
            + ".getDefaultInstance()";
      case INT:
        return "0";
      case LONG:
      ...
      case STRING:
        return "\"" + nullLiteral + "\"";
      case ENUM:
        return PromotedPbFormatUtils.getFullJavaName(fieldDescriptor.getEnumType())
            + ".values()[0]";
      case FLOAT:
        return "0.0f";
      case DOUBLE:
  ...
  /**
   * This method will be called from serializer of flink array/map type because flink contains both
   * array/map type in array format. Map/Array cannot contain null value in pb object then we must
   * do conversion in case of null values in map/array type.
   *
   * @param flinkArrDataVar code phrase represent arrayData of arr type or keyData/valueData in map
   *     type.
  ...
  public static String convertFlinkArrayElementToPbWithDefaultValueCode(
    ...
    String protoTypeStr = PromotedPbCodegenUtils.getTypeStrFromProto(elementPbFd, false);
    String dataTypeStr = PromotedPbCodegenUtils.getTypeStrFromLogicType(elementDataType);
    appender.appendLine(protoTypeStr + " " + resultPbVar);
    appender.begin("if(" + flinkArrDataVar + ".isNullAt(" + iVar + ")){");
    appender.appendLine(
        resultPbVar
            + "="
            + PromotedPbCodegenUtils.pbDefaultValueCode(elementPbFd, pbFormatContext));
    appender.end("}else{");
    appender.begin();
    appender.appendLine(dataTypeStr + " " + flinkElementVar);
    String flinkContainerElementCode =
        PromotedPbCodegenUtils.flinkContainerElementCode(flinkArrDataVar, iVar, elementDataType);
    appender.appendLine(flinkElementVar + " = " + flinkContainerElementCode);
    PbCodegenSerializer codegenSer =
        PromotedPbCodegenSerializeFactory.getPbCodegenSer(
  ...
  public static String convertFlinkArrayElementToPbMapEntry(
      String flinkArrDataVar,
      String iVar,
      String pbCollectionVar,
      String resultPbEntryVar,
      FieldDescriptor entryPbFd,
      LogicalType elementDataType,
      PbFormatContext pbFormatContext,
      int indent)
      throws PbCodegenException {
    ...
    String flinkElementVar = "elementVar" + uid;
    PbCodegenAppender appender = new PbCodegenAppender(indent);
    FieldDescriptor keyPdFd = entryPbFd.getMessageType().getFields().get(0);
    FieldDescriptor valuePdFd = entryPbFd.getMessageType().getFields().get(1);
    String protoKeyTypeStr = PromotedPbCodegenUtils.getTypeStrFromProto(keyPdFd, false);
    String protoValueTypeStr = PromotedPbCodegenUtils.getTypeStrFromProto(valuePdFd, false);
    String dataTypeStr = PromotedPbCodegenUtils.getTypeStrFromLogicType(elementDataType);
    appender.appendLine(
        "Entry<" + protoKeyTypeStr + ", " + protoValueTypeStr + "> " + resultPbEntryVar);
    appender.begin("if(!" + flinkArrDataVar + ".isNullAt(" + iVar + ")){");
    appender.begin();
    appender.appendLine(dataTypeStr + " " + flinkElementVar);
    String flinkContainerElementCode =
    ...
    PbCodegenSerializer entryCodegenSer =
        PromotedPbCodegenSerializeFactory.getPbCodegenSer(
            entryPbFd, elementDataType, pbFormatContext);
    String entryCode =
        entryCodegenSer.codegen(resultPbEntryVar, flinkElementVar, appender.currentIndent());
    appender.appendSegment(entryCode);
    appender.appendLine(
        pbCollectionVar + ".put(" + resultPbEntryVar + ".key, " + resultPbEntryVar + ".value)");
    appender.end("}");
    return appender.code();
  }

  public static Class<?> compileClass(ClassLoader classloader, String className, String code)
      throws ClassNotFoundException, PbCodegenException {
