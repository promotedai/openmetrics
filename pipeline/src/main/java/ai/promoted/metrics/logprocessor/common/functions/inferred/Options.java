...
public abstract class Options implements Serializable {
    ...
    public abstract static class Builder {
        /**
         * If true, skips the join and passes the RHS through.
         * Instead of removing the inferred ref operators from the job graph, we can just skip the join in the operator.
         * This has two benefits:
         * 1. It'll be easier to support multi-tenancy if we don't change the Flink graph per platform.
         * 2. We still need to flatten TinyDeliveryLog into TinyEvents.  The code for this is already in
         * BaseInferred.  Even if we skip inferred references, we'd still need an operator to flatten the records.
         */
