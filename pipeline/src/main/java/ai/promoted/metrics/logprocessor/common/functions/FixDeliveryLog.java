 ...
 * Fixes DeliveryLog.
 *
 * Older DeliveryLogs do not have insertionId set on Response Insertions.  This class creates insertion IDs, if they
 * do not exist, from "{requestId}-{position}-{contentId}".
 * We can remove this function after we launch and fix and wait enough time such that we won't want to backfill
 * over this range.
 *
 * TODO(PRO-857): remove this hack.
 */
public class FixDeliveryLog implements MapFunction<DeliveryLog, DeliveryLog> {
    ...
    private static boolean hasMissingResponseInsertionIds(DeliveryLog deliveryLog) {
        return deliveryLog.getResponse().getInsertionList().stream().anyMatch(insertion -> insertion.getInsertionId().isEmpty());
    }

    private static DeliveryLog.Builder fillInMissingResponseInsertionIds(DeliveryLog.Builder builder) {
    ...
    /**
     * Returns if {@param deliveryLog} is the v1 optimized SDK DeliveryLog format.  The v1 optimized format saved
     * request and response insertions as response insertions with extra properties.  This was originally done to save
     * space since we were going to pass CMS data through Insertions.  We'll change this because (1) we eventually need
     * to support no request insertions (Promoted retrieval) and (2) this format leads to more bugs.
     *
     * Reminder that the SDK DeliveryLog doesn't reorder the request insertions.
     */
    ...
    private static DeliveryLog.Builder convertFromV1OptimizedSdkDeliveryLog(DeliveryLog.Builder builder) {
        // Copy the merged response insertions to request insertions.  Strip out position since that's the resposne
        // insertion field.
        ...
        for (Insertion responseInsertion : responseInsertions) {
            requestBuilder.addInsertion(responseInsertion.toBuilder()
                    .clearPosition());
        }
        ...
        for (Insertion responseInsertion : responseInsertions) {
            responseBuilder.addInsertion(Insertion.newBuilder()
