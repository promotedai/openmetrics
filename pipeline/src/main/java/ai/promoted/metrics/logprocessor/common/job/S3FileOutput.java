...
public class S3FileOutput implements FlinkSegment {
    ...
    @Option(names = {"-o", "--s3OutputDirectory"}, defaultValue = "", description = "Name of the s3 output (e.g. 's3a://my-bucket/').")
    public String s3OutputDirectory = "";

    // TODO - deprecate and delete.
    @Option(names = {"-s", "--s3Bucket"}, defaultValue = "", description = "Name of the s3 bucket.  --s3OutputDirectory is higher priority.  Prefer to use that instead.")
    public String deprecatedS3Bucket = "";

    @FeatureFlag
    @Option(names = {"--no-logAvro"}, negatable = true, description = "Write Avro files with 'fixed' non-recursive schemas.")
    public boolean logAvro = true;

    @FeatureFlag
    @Option(names = {"--logParquet"}, negatable = true, description = "Write Parquet files with non-recursive schemas.")
    public boolean logParquet = false;

    @FeatureFlag
    @Option(names = {"--compressionCodecName"}, description = "Compression codec for parquet. Valid values: ${COMPLETION-CANDIDATES}. Default SNAPPY")
    public CompressionCodecName compressionCodecName = CompressionCodecName.SNAPPY;

    @FeatureFlag
    @Option(names = {"--sideOutputDebugLogging"}, negatable = true, description = "Whether to write side output debug logging.  Default=false")
    public boolean sideOutputDebugLogging = false;

    @FeatureFlag
    @Option(names = {"--disableS3Sink"}, description = "Flag to disable specific S3 sinks.  This is a flag in case writing causes performance issues.  Defaults to empty.")
    ...
    @VisibleForTesting
    public ArrayList<String> sinkTransformationUids = new ArrayList<>();

    // Only used for avoiding duplicated job label state.
    private BaseFlinkJob job;

    public S3FileOutput(BaseFlinkJob job) {
        this.job = job;
    }
    ...
    public void validateArgs() {
        ...
        LOGGER.info("outputS3Directory={}", getOutputS3Dir().build().toString());
    }
    ...
    public S3Path.Builder getOutputS3Dir(String... subdirs) {
        return getOutputS3Dir(ImmutableList.copyOf(subdirs));
    }

    /** DO NOT pass in subdirs with path delimiters like '/'.  This does it for you. */
    public S3Path.Builder getOutputS3Dir(Iterable<String> subdirs) {
        S3Path.Builder builder = getS3PathBuilder();
    ...
    private S3Path.Builder getS3PathBuilder() {
        S3Path.Builder builder = S3Path.builder();
        if (!s3OutputDirectory.isEmpty()) {
            builder = builder.setRoot(s3OutputDirectory);
        } else if (!deprecatedS3Bucket.isEmpty()) {
            builder = builder.setRoot(Constants.getS3BucketUri(deprecatedS3Bucket));
        } else {
    ...
    public <T extends GeneratedMessageV3> ImmutableList<Transformation> sink(
        ...
        SerializableToLongFunction<T> getTimestamp = (row) -> getTiming.apply(row).getEventApiTimestamp();
    ...
    public <T extends GeneratedMessageV3> ImmutableList<Transformation> sink(
        ...
        if (logParquet) {
            ...
            sink(getParquetWriterFactory(clazz, compressionCodecName), getTimestamp, input, pathBuilder).ifPresent(xforms::add);
        }
        return xforms.build();
    }
    ...
    public Optional<Transformation> outputDebugLogging(
            String name,
            DataStream<GenericRecord> logStream,
            Schema logSchema) {
        if (!sideOutputDebugLogging) return Optional.empty();

        S3Path outputDirectory = getOutputS3Dir("etl-side", "debug", name).build();
        ...
        BulkWriter.Factory<GenericRecord> factory = S3FileOutput.<GenericRecord>getGenericRecordParquetWriterFactory(logSchema);
        job.add(logStream.sinkTo(forBulkFormat(outputDirectory, factory, r -> (long)r.get("timestamp"))), uid);

        sinkTransformationUids.add(uid);
        return Optional.of(logStream.getTransformation());
    }

    public <T extends org.apache.avro.specific.SpecificRecordBase> Optional<Transformation> outputSpecificAvroRecordParquet(
    ...
    private static <T> FileSink<T> forBulkFormat(S3Path path, BulkWriter.Factory<T> factory, SerializableToLongFunction<T> getTimestamp) {
    ...
    private static <T> BulkWriter.Factory<T> getGenericRecordParquetWriterFactory(Schema schema) {
        ParquetBuilder<T> builder = (out) -> AvroParquetWriter.<T>builder(out)
            .withSchema(schema)
    ...
    // Currently has issues with serialization due to the underlying proto objects not having mappings in avro.
    private static <T extends GeneratedMessageV3> BulkWriter.Factory<T> getAvroParquetWriterFactory(Class<T> clazz) {
        ParquetBuilder<T> builder = (out) -> AvroParquetWriter.<T>builder(out)
    ...
    private static <T extends GeneratedMessageV3> BulkWriter.Factory<T> getAvroWriterFactory(Class<T> protoClass) {
    ...
    private static <T extends GeneratedMessageV3> BulkWriter.Factory<T> getParquetWriterFactory(Class<T> clazz, CompressionCodecName compressionCodecName) {
    ...
    private static class MyParquetWriterBuilder<T extends Message>
        ...
        public MyParquetWriterBuilder(OutputFile outputFile, Class<T> clazz, CompressionCodecName compressionCodecName) {
            super(outputFile);
            ...
            return new ProtoWriteSupport<>(clazz) {
                ...
                public void write(T record) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("class is " + clazz + ", my record: " + record.toString(), e);
                    }
                }
            };
        }
