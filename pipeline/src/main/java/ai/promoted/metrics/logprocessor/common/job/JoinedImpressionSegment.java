...
/**
 * A FlinkSegment that provides joined impressions as a source.  It's a separate FlinkSegment
...
public class JoinedImpressionSegment implements FlinkSegment {
    ...
    @CommandLine.Option(names = {"--keepFirstJoinedImpressionDuration"}, defaultValue = "PT6H", description = "The duration to keep track of recent JoinedImpressions.  This is used to de-duplicate joined inputs.  Default=PT6H.  Java8 Duration parse format.")
    ...
    public DataStream<JoinedEvent> getDeduplicatedJoinedImpression(DataStream<JoinedEvent> joinedImpression) {
        return baseFlinkJob.add(
                joinedImpression.keyBy(KeyUtil.joinedImpressionKey)
                        .process(new KeepFirstRow<>("joined-impression", keepFirstJoinedImpressionDuration),
