...
public class ImpressionActionProcessFunction extends ContentId<TinyEvent> {
    // Used as a separator when creating the primary key.  See the comment in the constructor for more details.
    // 0x01 = (^A)
    ...
    public ImpressionActionProcessFunction(Options options, List<String> otherDimensionIds) {
        super("impression x action",
                (TinyEvent event) -> !event.getImpressionId().isEmpty(),
                TinyFlatUtil::getAllContentIds,
                // The join job splits TinyActions by Shopping CartItem.  It keeps the TinyEvent.action_id the same.
                ...
                false,
                options);
        this.otherDimensionIds = otherDimensionIds.stream().map(StringUtil::hash).collect(ImmutableList.toImmutableList());
    }

    protected ImmutableList<String> getLeftJoinIds(TinyEvent flat) {
        ImmutableList.Builder<String> builder = ImmutableList.<String>builderWithExpectedSize(5 + otherDimensionIds.size())
                .add(flat.getImpressionId())
                .add(flat.getInsertionId())
                .add(flat.getRequestId())
                .add(flat.getViewId())
                .add(flat.getContentId());
        // Include other content IDs in join scopes so we can attribute actions for parent content types.
        otherDimensionIds.forEach(key -> {
    ...
    // TODO - might be able to optimize this just for CHECKOUT and ACTION (or when only contentId is specified).
