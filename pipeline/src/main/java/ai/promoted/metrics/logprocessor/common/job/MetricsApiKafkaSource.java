...
// If we need to make this class serializable, we need to figure out how to handle filteredLogRequestStream.
public class MetricsApiKafkaSource implements FlinkSegment {

    @Option(names = {"--logRequestTopicOverride"}, defaultValue = "", description = "Overrides the input LogRequest " +
            "Kafka topic.")
    public String logRequestTopicOverride = "";

    @Option(names = {"--maxLogRequestOutOfOrderness"}, defaultValue = "PT1S", description = "The maxOutOfOrderness " +
            "for the LogRequest kafka. Default=PT1S")
    public Duration maxLogRequestOutOfOrderness = Duration.parse("PT1S");

    // Is a separate flag from `--kafkaDataset` because the input kafka topic uses a deprecated dataset.
     ...
     * MetricsApiKafkaSource is scoped to a job and control flags.  This class is scoped to a stream of LogRequests.
     */
    public final class SplitSources {
        ...
        private SplitSources(SingleOutputStreamOperator<LogRequest> logRequestStream) {
            this.logRequestStream = job.add(logRequestStream.process(new LogRequestFilter()), "split-log-request");
        }
    ...
    String getInputLogRequestTopic() {
        return firstNotEmpty(logRequestTopicOverride, kafkaSegment.getKafkaTopic(
