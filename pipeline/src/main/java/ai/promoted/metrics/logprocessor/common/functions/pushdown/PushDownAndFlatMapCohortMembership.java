...
/**
 * Copies {@link LogRequest}'s {@link CohortMembership} and fills in default base values.
...
public class PushDownAndFlatMapCohortMembership extends BasePushDownAndFlatMap<CohortMembership> {
    ...
    public void flatMap(LogRequest logRequest, Consumer<CohortMembership> out) {
        ...
        logRequest.getCohortMembershipList().stream()
                .map(cohortMembership -> pushDownFields(cohortMembership, logRequest, lowerCaseBatchLogUserId))
                .forEach(out);
    }

    public static CohortMembership pushDownFields(CohortMembership cohortMembership, LogRequest batchValue, String lowerCaseBatchLogUserId) {
