...
public class ValidateDeliveryLog extends BaseValidate<DeliveryLog> {
    public void processElement(DeliveryLog deliveryLog, ProcessFunction<DeliveryLog, DeliveryLog>.Context ctx, Collector<DeliveryLog> out) throws Exception {
        ...
        if (hasMismatchedMatrixHeaderLength(deliveryLog)) {
            errors.add(createError(deliveryLog, ErrorType.MISMATCHED_MATRIX_HEADER_LENGTH, Field.MULTIPLE));
        }
