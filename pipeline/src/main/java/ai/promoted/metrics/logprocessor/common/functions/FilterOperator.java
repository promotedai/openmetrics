...
/**
 * A simple filter Operator that splits the output stream to predicate matching (main output) and non-matching
 * (side output) streams.
 */
public class FilterOperator<T> extends ProcessFunction<T, T> {
    ...
    public void processElement(T record, ProcessFunction<T, T>.Context ctx, Collector<T> out) throws Exception {
