...
/**
 * KeyedCoProcessFunction that only emits the "left" stream iff it has seen some
 * part of it (using the rightExtractor function) on the "right" stream.
 *
 * This function skips nulls on both the left and right streams.
 */
public class RightSeenOutput<LEFT, RIGHT> extends KeyedCoProcessFunction<RIGHT, LEFT, RIGHT, LEFT> {
    ...
    public void processElement1(LEFT left, Context ctx, Collector<LEFT> out) throws Exception {
        LOGGER.trace("{} {} {} processElement1: {}", ctx.timestamp(), ctx.timerService().currentWatermark(), ctx.getCurrentKey(), left);
        if (left == null) return;
        delayed.add(left);
    ...
    public void onTimer(long ts, OnTimerContext ctx, Collector<LEFT> out) throws Exception {
        LOGGER.trace("{} {} {} onTimer", ts, ctx.timerService().currentWatermark(), ctx.getCurrentKey());
        if (Boolean.TRUE.equals(seen.value())) {
            for (LEFT left : delayed.get()) {
                LOGGER.trace("{} {} contained: {} output: {}", ctx.timestamp(), ctx.timerService().currentWatermark(), ctx.getCurrentKey(), left);
                out.collect(left);
            }
        }
        delayed.clear();
    ...
    public void processElement2(RIGHT right, Context ctx, Collector<LEFT> out) throws IOException {
        LOGGER.trace("{} {} {} processElement2: {}", ctx.timestamp(), ctx.timerService().currentWatermark(), ctx.getCurrentKey(), right);
