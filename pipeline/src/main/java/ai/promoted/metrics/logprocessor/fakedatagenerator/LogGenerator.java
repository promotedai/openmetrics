...
@Command(name = "fakeloggenerator", mixinStandardHelpOptions = true, version = "fakeloggenerator 1.0.0",
...
public class LogGenerator implements Callable<Integer> {
    ...
    @Option(names = {"--metricsApiKafkaDataset"}, defaultValue = Constants.DEPRECATED_DEFAULT_KAFKA_DATASET,
    ...
    @Option(names = {"-i", "--responseInsertionsPerRequest"}, defaultValue = "20",
    ...
    @Option(names = {"--emptyLogUserIdRate"}, defaultValue = "0.0",
    ...
    @Option(names = {"--requestContentType0"}, defaultValue = "ITEM",
            description = "The type of content for the first request page.")
    private ContentType requestContentType0 = ContentType.ITEM;

    @Option(names = {"--requestContentType1"}, description = "The type of content for the request page.")
    ...
    private String getLogRequestTopic() {
        return firstNonEmpty(logRequestTopicOverride,
    ...
    public Integer call() throws Exception {
        ...
        ProtoKafkaSerializationSchema<LogRequest> logRequestSerializer = new ProtoKafkaSerializationSchema<>(
                topic,
            ...
            current.getAllFields().entrySet().stream()
                    ...
                    .forEach(entry -> {
                        ...
                        descriptorToCount.put(
                                entry.getKey(),
                                descriptorToCount.getOrDefault(entry.getKey(), 0L) + count);
                    });
        ...
        long subrecordTotal = descriptorToCount.values().stream().mapToLong(value -> value.longValue()).sum();
        Map<String, Long> readableDescriptorToCount = descriptorToCount.entrySet().stream()
    ...
    private LogRequestIterator createLogRequestIterator() {
        ...
        if (startEventApiTimestamp == 0L) {
            ...
            eventApiTimestampSupplier = () -> Instant.ofEpochMilli(startEventApiTimestamp + Instant.now().toEpochMilli() - nowStart.toEpochMilli());
        }
        return new LogRequestIterator(LogRequestFactory.setTestMarketplaceFields(LogRequestIteratorOptions.builder())
                .setUsers(users)
                ...
                .setRequestInsertionSurface0(FakeInsertionSurface.create(requestContentType0, true, true))
                .setRequestInsertionSurface1(
                        requestContentType1.isPresent()
                                ? Optional.of(FakeInsertionSurface.create(requestContentType1.get(), true, true))
                                : Optional.empty())
                ...
                .setLogUserUuidSupplier((userId) -> {
                    if (LogRequestIterator.matchesRateOption(emptyLogUserIdRate, userId, "setLogUserUuidSupplier")) {
                        return "";
                    } else {
