...
/**
 * Utilities for converting from Protos to Avro. This is in the testing directory because it's not
 * very efficient.
 */
public class AvroProtoUtils {

  /** Writes proto messages to Avro file. */
  public static <T extends GeneratedMessageV3> void writeAvroFile(File file, Iterable<T> messages)
      throws IOException {
  ...
  public static <T extends GeneratedMessageV3> GenericRecord toAvroGenericRecord(T message)
      throws IOException {
  ...
  public static <T extends GeneratedMessageV3> Iterable<GenericRecord> toAvroGenericRecords(
      Iterable<T> messages) throws IOException {
    if (!messages.iterator().hasNext()) {
    ...
    try (DataFileReader<GenericRecord> dataFileReader =
        new DataFileReader<>(new SeekableByteArrayInput(out.toByteArray()), reader)) {
  ...
  public static <T extends GeneratedMessageV3> GenericRecord toFixedAvroGenericRecord(T message)
      throws IOException {
    return toFixedAvroGenericRecords(ImmutableList.of(message)).iterator().next();
  }

  /**
   * Converts multiple proto messages to Avro GenericRecords with Avro schema fixes for recursive
   * fields.
   */
  public static <T extends GeneratedMessageV3> Iterable<GenericRecord> toFixedAvroGenericRecords(
      Stream<T> messages) throws IOException {
