...
public class PromotedProtobufDatumWriter<T> extends ProtobufDatumWriter<T> {
  /**
   * Unrecognized enum values get their own EnumValueDescriptors. This complicates the Proto Avro
   * code. To support this case, we cache the mapping from EnumValueDescriptors to symbols. There
   * doesn't appear to be clean API to differentiate between recognized and unrecognized. We'll use
   * a cache. EnumValueDescriptor equals/hashcode is the default Object implementations. This
   * appears to be safe based on the current Protobuf GeneratedMessageV3 code. If this structure
  ...
  public PromotedProtobufDatumWriter(Schema root, PromotedProtobufData protobufData) {
    ...
    descriptorToSymbol =
        CacheBuilder.newBuilder()
            ...
            .build(
                new CacheLoader<Descriptors.EnumValueDescriptor, String>() {
                  @Override
                  public String load(final Descriptors.EnumValueDescriptor descriptor)
                      throws Exception {
                    // Assumes core descriptors are reused.
                    Optional<Descriptors.EnumValueDescriptor> matchingDescriptor =
                        descriptor.getType().getValues().stream()
