...
public class DeliveryLogUtil {
    private final static Logger LOGGER = LogManager.getLogger(DeliveryLogUtil.class);

    /**
     * Returns a priority for how important the traffic is for combining and joining.  Higher is more important.
     * <ul>
     *     <li>3 = was used by the customer</li>
     *     <li>2 = shadow traffic.  We join in this but it's not as important as priority 3</li>
     *     <li>1 = test and dev</li>
     * </ul>
     */
    ...
    public static String getLogUserId(CombinedDeliveryLog combinedDeliveryLog) {
        ...
        if (combinedDeliveryLog.hasApi() && combinedDeliveryLog.hasSdk()) {
            ...
            if (!Objects.equal(apiLogUserId, sdkLogUserId)) {
                LOGGER.error("DeliveryLogUtil.getLogUserId(CombinedDeliveryLog) has mismatching platformIds, api.logUserId={}, sdk.logUserId={}",
                        apiLogUserId, sdkLogUserId);
            }
    ...
    // It's a little confusing for user_info to be retrieved from Request and platformId from the parent.
    ...
    public static long getPlatformId(CombinedDeliveryLog combinedDeliveryLog) {
        ...
        if (combinedDeliveryLog.hasApi() && combinedDeliveryLog.hasSdk()) {
            ...
            if (apiPlatformId != sdkPlatformId) {
                LOGGER.error("DeliveryLogUtil.getPlatformId(CombinedDeliveryLog) has mismatching platformIds, api.platformId={}, sdk.platformId={}",
                        apiPlatformId, sdkPlatformId);
            }
            if (apiPlatformId != 0L) {
