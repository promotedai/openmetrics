...
public class ValidateAction extends BaseValidate<Action> {
    ...
    @Override
    public void processElement(Action action, ProcessFunction<Action, Action>.Context ctx, Collector<Action> out) throws Exception {
        ...
        if (action.getImpressionId().isEmpty() && action.getInsertionId().isEmpty() && action.getContentId().isEmpty()
                && action.getCart().getContentsList().stream().allMatch(cartContent -> cartContent.getContentId().isEmpty())) {
        ...
        if (action.hasCart()) {
            ...
            if (action.getCart().getContentsList().stream().anyMatch(content -> content.getQuantity() == 0L)) {
            ...
            if (action.getCart().getContentsList().stream().anyMatch(content -> content.getContentId().isEmpty())) {
