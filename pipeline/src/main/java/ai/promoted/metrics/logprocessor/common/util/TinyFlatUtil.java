...
public final class TinyFlatUtil {
  public static TinyEvent.Builder mergeImpression(
      TinyEvent.Builder builder,
      TinyEvent rhs,
      BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger) {
    TinyEvent.Builder tinyImpressionBuilder =
        mergeTinyEvent(
  ...
  public static TinyEvent.Builder mergeAction(
      ...
      TinyEvent rhs,
      BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger) {
    TinyEvent.Builder tinyActionBuilder =
        mergeTinyEvent(
            RecordType.ACTION,
            builder,
            rhs,
    ...
    // When merging tiny events, the LHS values are usually more accurate.  E.g. we usually trust
    // IDs on
    ...
    //
    // PR - we could split the TinyEvent.content_id field into an impression_content_id and an
    // action_content_id.
  ...
  private static TinyEvent.Builder mergeTinyEvent(
    ...
    setter.setValue(
        Field.PLATFORM_ID, builder::setPlatformId, builder.getPlatformId(), rhs.getPlatformId());
    setter.setValue(
        Field.LOG_USER_ID, builder::setLogUserId, builder.getLogUserId(), rhs.getLogUserId());
    setter.setValue(Field.VIEW_ID, builder::setViewId, builder.getViewId(), rhs.getViewId());
    setter.setValue(
        Field.REQUEST_ID, builder::setRequestId, builder.getRequestId(), rhs.getRequestId());
    setter.setValue(
        Field.INSERTION_ID,
        builder::setInsertionId,
        builder.getInsertionId(),
        rhs.getInsertionId());
    setter.setValue(
        Field.IMPRESSION_ID,
        builder::setImpressionId,
        builder.getImpressionId(),
        rhs.getImpressionId());
    setter.setValue(
        Field.ACTION_ID, builder::setActionId, builder.getActionId(), rhs.getActionId());
  ...
  public static List<TinyEvent.Builder> createTinyFlatResponseInsertions(
    ...
    setter.setValue(
        Field.PLATFORM_ID,
        baseBuilder::setPlatformId,
        baseBuilder.getPlatformId(),
        deliveryLog.getPlatformId());
    setter.setValue(
        Field.LOG_USER_ID,
        baseBuilder::setLogUserId,
        baseBuilder.getLogUserId(),
        deliveryLog.getLogUserId());
    setter.setValue(
  ...
  public static ImmutableSet<String> getAllContentIds(TinyEvent event) {
    ImmutableSet.Builder<String> builder =
        ImmutableSet.builderWithExpectedSize(1 + event.getOtherContentIdsCount());
