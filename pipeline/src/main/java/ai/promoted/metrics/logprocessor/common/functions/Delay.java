...
/**
 * Delays a stream of events by a duration.  This is currently used as a hack so we can delay some synthetic User
 * records for real ones.  I hope we can remove this after changing the User DB join structure.
 */
public class Delay<T> extends KeyedProcessFunction<Tuple2<String, Long>, T, T> {
    ...
    public void open(Configuration config) throws Exception {
        super.open(config);
        timestampToList = getRuntimeContext().getMapState(
                // TODO(PRO-1683) - add caches back in.
                new MapStateDescriptor<>(
                        stateLabel + "-delay",
                        Types.LONG,
                        Types.LIST(TypeInformation.of(valueClass))));
    }
}
