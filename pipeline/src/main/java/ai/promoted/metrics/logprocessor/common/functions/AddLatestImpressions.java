...
/**
 * A function that coprocesses {@code FlatActions} and {@code FlatImpressions} to join a list of the latest impressions
 * onto the flat actions.  This list of impressions is used by AWS Personalize as a negative signal.  This function
 * filters out contentIds with positive behavior (action events).
 *
 * The logic is pretty complex:
 * 1. buffers actions and impressions temporarily.
 * 2. uses timers to delay output the action events with the {@code LatestImpressions}
 * 3. removes contentId that (a) has actions on it and (b) duplicated.
 *
 * K = log_user_id.
 * IN1 = *JoinedEvent for joined-action
 * IN2 = *JoinedEvent for joined-impression
 * OUT = *JoinedEvent for joined-action with recent impressions
 */
public class AddLatestImpressions extends KeyedCoProcessFunction<Tuple2<Long, String>, JoinedEvent, JoinedEvent, JoinedEvent> {
    ...
    private static final long SESSION_DURATION_MILLIS = Constants.SESSION_DURATION_HOURS * MILLIS_PER_HOUR;
    ...
    private static Comparator<LatestImpression> LATEST_IMPRESSION_TIMESTAMP_COMPARATOR = Comparator.comparingLong(AddLatestImpressions::getLatestImpressionTimestamp);

    // Stores Actions temporarily so we can handle late events.
    @VisibleForTesting
    transient ListState<JoinedEvent> actions;
    // Stores Impressions so we can join them into Actions.
    @VisibleForTesting
    transient ListState<LatestImpression> impressions;
    // Stores a smaller version of Actions so we can exclude positive content events from the joined output.
    // Overloads LatestImpression.  Timestamp is the action timestamp.
    @VisibleForTesting
    transient ListState<LatestImpression> actionedImpressions;
    ...
    // Max number of Impressions to keep in our state.  We want extra Impressions so we have backups in case
    ...
    public AddLatestImpressions(Duration maxOutOfOrderness, boolean runAsserts) {
        ...
        // maxStateImpressions = factoring in buffer in case the user actions a lot of items they've seen.
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        ListStateDescriptor<JoinedEvent> actionsDescriptor =
                new ListStateDescriptor<JoinedEvent>(
                        "actions",
        ...
        ListStateDescriptor<LatestImpression> impressionsDescriptor =
                new ListStateDescriptor<>(
                        "impressions",
                        TypeInformation.of(LatestImpression.class));
     ...
     *
     * We slightly prefer clientLogTimestamp over eventApiTimestamp for the
     * timestamp.  Both have issues:
     * 1. clientLogTimestamp - relative ordering is probably more accurate.
     *    Harder to trust and it won't always be set (which is why I fall back
     *    to event api ts).
     * 2. eventApiTimestamp - we can trust more but delays on the client side
     *    can cause weird smaller order issues.
     *
     * If we eventually want both, we could add another field to
     * LatestImpression to have a different timestamp field. We'd still want to
     * pick one and order by it.
     */
    @VisibleForTesting
    ...
    // Actions.
    @Override
    public void processElement1(JoinedEvent joinedAction, Context context, Collector<JoinedEvent> collector) throws Exception {
        // Add actions to the state.  Most of the work is done in the timer after waiting for more events.
        actions.add(joinedAction);

        // Track the latest impressions that are actioned on.  We'll use this list to exclude impressions later.
        long timestamp = context.timestamp();
        Action action = joinedAction.getAction();
        JoinedIdentifiers ids = joinedAction.getIds();
        actionedImpressions.add(LatestImpression.newBuilder()
        ...
        long sessionCleanupTimestamp = ((timestamp / MILLIS_PER_HOUR) + SESSION_CLEANUP_HOURS) * MILLIS_PER_HOUR;
        context.timerService().registerEventTimeTimer(sessionCleanupTimestamp);
    }

    // Impressions.
    @Override
    public void processElement2(JoinedEvent joinedImpression, Context context, Collector<JoinedEvent> collector) throws Exception {
        // Convert these to LatestImpression (smaller version).
        long timestamp = context.timestamp();
        Impression impression = joinedImpression.getImpression();
        JoinedIdentifiers ids = joinedImpression.getIds();
        impressions.add(LatestImpression.newBuilder()
                .setImpressionId(ids.getImpressionId())
        ...
        long smallCleanupTimestamp = ((timestamp / MILLIS_PER_MINUTE) + PRUNE_IMPRESSIONS_CLEANUP_MINUTES) * MILLIS_PER_MINUTE;
    ...
    private static boolean isSorted(List<LatestImpression> sortedImpressions, int index) {
        if (index < 2) {
            return true;
        } else if (LATEST_IMPRESSION_TIMESTAMP_COMPARATOR.compare(sortedImpressions.get(index - 2), sortedImpressions.get(index - 1)) > 0) {
            return false;
        } else {
    ...
    /**
     * Copies {@param sortedImpressions} and filters in reverse order.  {@code FilterResult} is used to support
     * including items that do not count towards {@code maxKeepSize}.
     */
    private List<LatestImpression> copyAndFilterInReverseOrder(
        ...
        // We want to remove older content.  It's easier to do this by iterating over the list the opposite way and
        // then flip it.
    ...
    @Override
    public void onTimer(long timestamp, OnTimerContext ctx, Collector<JoinedEvent> actionOut) throws Exception {
        super.onTimer(timestamp, ctx, actionOut);

        // For now, we'll just sort the impression list each timer.  This uses TimSort so it should be pretty
        // efficient.
        List<LatestImpression> sortedImpressions = toStream(impressions)
        ...
        for (JoinedEvent joinedAction : actions.get()) {
            ...
            // We're not using the server-side time because (1) it's not available on the Action proto and (2)
            ...
            if (actionFlinkTimestamp <= timestamp - maxOutOfOrderness.toMillis()) {
                ...
                Set<String> seenContentIds = toStream(actionedImpressions)
                        .filter(actionImpression -> getLatestImpressionTimestamp(actionImpression) <= actionTimestamp)
                        .map(LatestImpression::getContentId)
                        .collect(Collectors.toSet());

                List<LatestImpression> actionImpressions = copyAndFilterInReverseOrder(
                        sortedImpressions,
                        (impression) -> {
                            // We don't need a lateDurationMillis check here since we already checked that the action is
                            // safely out of the late time period.
                            if (getLatestImpressionTimestamp(impression) <= actionTimestamp && seenContentIds.add(impression.getContentId())) {
                                return FilterResult.KEEP_AND_COUNT;
                            }
                            return FilterResult.REMOVE;
                        },
                ...
                actionOut.collect(joinedAction.toBuilder()
                        .setLatestImpressions(latestImpressionBuilder)
                        .build());
            } else {
