...
/**
 * <p>Merges sorted user updates and outputs the "effective user" at the time set in "advanceTo".</p>
 * <p>
 *     UpdatedUserBuilder is used by the UserJoin function to get an effective final User record for a singe LUID.
 *     This allows us to do a hacky time lookup.
 *     This class helps merging in synthetic User records (userIds on DeliveryLogs) and clean up older state.
 *     This does not support undoing the "advanceTo" function.
 * </p>
 * <p>TODO - might make sense to advance more in advanceTo if an upcoming User has properties.</p>
 */
class UserUpdateMerger {
    ...
    /**
     * @param userUpdates list of user updates sorted by logTimestamp.  Not checking here since this class is package protected and avoid the performance hit.
     */
    ...
    public void advanceTo(long timestamp) {
        ...
        while (nextUser < userUpdates.size() && timestamp >= userUpdates.get(nextUser).getTiming().getLogTimestamp()) {
            if (userBuilder == null) {
                ...
                if (userUpdate.hasUserInfo()) {
                    ...
                    if (runAssert && userBuilder.hasUserInfo()) {
                        if (!userBuilder.getUserInfo().getLogUserId().isEmpty()
                                ...
                                && !userBuilder.getUserInfo().getLogUserId().equals(updateUserInfo.getLogUserId())) {
                            throw new IllegalStateException("Encountered unexpected change in logUserIds; userUpdates=" + userUpdates);
                        }
                        if (!userBuilder.getUserInfo().getUserId().isEmpty()
                            ...
                            throw new IllegalStateException("Encountered unexpected change in userIds; userUpdates=" + userUpdates);
                        }
                    }
