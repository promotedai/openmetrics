...
public class FlatMapUtil {
  ...
  private static Stream<Map.Entry<String, Object>> flattenMap(
      String parentKey, Map<String, Object> map) {
    return map.entrySet().stream()
        .flatMap(
            entry ->
                flattenEntry(
                    new AbstractMap.SimpleEntry<>(
                        parentKey + "/" + entry.getKey(), entry.getValue())));
  }

  private static Stream<Map.Entry<String, Object>> flattenList(List<?> list) {
    return IntStream.range(0, list.size())
        .mapToObj(
            i -> new AbstractMap.SimpleEntry<String, Object>(Integer.toString(i), list.get(i)))
