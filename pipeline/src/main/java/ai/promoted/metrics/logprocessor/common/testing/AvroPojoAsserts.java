...
/**
 * Asserts for Avro files and plain old java objects (POJOs).
 * Prefer using AvroProtoUtils.  It is easier to reuse common test objects that are protos across systems.
 * POJO comparisons will be used more for internal testing of our frameworks.
 */
public interface AvroPojoAsserts {
    ...
    static Object fromAvroToPojo(Object value) {
        ...
        Preconditions.checkArgument(
                ...
                "Avro GenericRecord value class is unsupported: %s", value.getClass());
        if (value instanceof ByteBuffer) {
            ...
            return ((GenericData.Array) value).stream().map(AvroPojoAsserts::fromAvroToPojo).collect(Collectors.toList());
    ...
    static Set<Map<String, Object>> getRecordMaps(File[] files) throws IOException {
        return AvroAsserts.loadAvroRecords(files).stream().map(AvroPojoAsserts::fromAvroRecordToMap).collect(Collectors.toUnmodifiableSet());
    }

    static Set<Map<String, Object>> getAvroParquetRecordMaps(File[] files) throws IOException {
        return AvroAsserts.loadAvroParquetRecords(files).stream().map(AvroPojoAsserts::fromAvroRecordToMap).collect(Collectors.toUnmodifiableSet());
    }
    ...
    static Map<String, Object> addTypeLayer(Map<String, Object> map) {
        ...
        map.entrySet().forEach(entry -> {
            ...
            if (entry.getValue() instanceof Map) {
                value = ((Map<String, Object>) entry.getValue()).entrySet()
                        .stream()
                        .collect(Collectors.toMap(Map.Entry::getKey, e -> new ObjectWrapper(e.getValue())));
            } else {
     ...
     */
    static void assertAvroFiles(Set<Map<String, Object>> expectedRecords, File[] files) throws IOException {
        Set<Map<String, Object>> actualRecords = getRecordMaps(files);
        assertAvroFiles(expectedRecords, actualRecords);
    }

    static void assertAvroParquetFiles(Set<Map<String, Object>> expectedRecords, File[] files) throws IOException {
        Set<Map<String, Object>> actualRecords = getAvroParquetRecordMaps(files);
        assertAvroFiles(expectedRecords, actualRecords);
    }

    static void assertAvroFiles(Set<Map<String, Object>> expectedRecords, Set<Map<String, Object>> actualRecords) throws IOException {
        ...
        Sets.SetView<Map<String, Object>> expectedDiffs = Sets.difference(expectedRecords, actualRecords);
     ...
     */
    static void assertMapEquals(Map<String, Object> expectedRecord, Map<String, Object> actualRecord) throws IOException {
