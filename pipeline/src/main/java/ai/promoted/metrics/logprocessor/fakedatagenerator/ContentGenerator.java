...
@Command(name = "fakecontentgenerator", mixinStandardHelpOptions = true, version = "fakecontentgenerator 1.0.0",
...
public class ContentGenerator implements Callable<Integer> {
    private final static Logger LOGGER = LogManager.getLogger(ContentGenerator.class);

    @Option(names = {"--contentApiRootUrl"}, defaultValue = "",
            description = "Format matches `http://ee1a-67-168-60-51.ngrok.io/` or `http://localhost:5150/`.")
    private String contentApiRootUrl = "";

    @Option(names = {"--contentApiKey"}, defaultValue = "",
    ...
    public Integer call() throws Exception {
        validateArgs();
        ContentDB contentDb = ContentDBFactory.create(LogRequestIterator.PLATFORM_ID, LogRequestIteratorOptions.DEFAULT_CONTENT_OPTIONS);
        writeContents(contentDb);
        return 0;
    }

    private void validateArgs() {
        Preconditions.checkArgument(!contentApiRootUrl.isEmpty(), "--rootContentApiUrl must be specified");
        Preconditions.checkArgument(contentApiRootUrl.endsWith("/"), "--rootContentApiUrl should end with a `/`");
    ...
    private void writeContents(ContentDB contentDb) throws Exception {
        ...
        for (List<Content> contentBatch : contentBatches) {
            Map<String, Map<String, String>> batchMap = contentBatch.stream()
    ...
    private void writeContentBatch(Map<String, Map<String, String>> batch) throws Exception {
        ...
        try {
            ...
            LOGGER.info("Wrote content batch responseCode={}, size={}, contentIds={}",
                    connection.getResponseCode(), batch.size(), batch.keySet());
