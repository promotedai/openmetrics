...
public abstract class BaseValidate<T> extends ProcessFunction<T, T> {
    ...
    public static final OutputTag<ValidationError> INVALID_TAG = new OutputTag<ValidationError>("invalid"){};

    protected static <T> void outputErrorsOrRecord(T record, ImmutableList.Builder<ValidationError> errors, ProcessFunction<T, T>.Context ctx, Collector<T> out) throws Exception {
        outputErrorsOrRecord(record, errors.build(), ctx, out);
    }

    protected static <T> void outputErrorsOrRecord(T record, List<ValidationError> errors, ProcessFunction<T, T>.Context ctx, Collector<T> out) throws Exception {
        if (errors.isEmpty()) {
            out.collect(record);
        } else {
    ...
    /**
     * Creates a base {@link ValidationError.Builder} for the record by setting the fields specific
     * to the record.  This is called by {@code createError} which fills in error specific fields.
     */
