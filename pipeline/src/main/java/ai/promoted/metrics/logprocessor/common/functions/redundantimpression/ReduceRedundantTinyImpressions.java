...
/**
 * Used to filter out redundant impressions. Changes actions that reference redundant impressions to
 * an impression that this Function outputs.
 *
 * <p>The inputs need to be keyed by the grouping dimensions for reduction.
 *
 * <p>The {@code reducedImpression} is the earliest impression until an impression is encountered
 * after {@code replaceReducedImpressionAfter}. The {@code reducedImpression} is kept for {@code 2 *
 * ttl} in case a redundant impression and action are encountered that would push the action outside
 * of the {@code ttl}. This is not optimal but keeps the logic simple.
 *
 * @param <K> The grouping dimension for reduction IN = TinyEvent for both flat impressions and
 *     actions OUT = TinyEvent for both flat impressions and actions
 */
public class ReduceRedundantTinyImpressions<K>
    extends KeyedProcessFunction<K, TinyEvent, TinyEvent> {
  ...
  public static final OutputTag<TinyEvent> REDUNDANT_IMPRESSION =
      new OutputTag<TinyEvent>("redundant-impression") {};

  @VisibleForTesting transient ValueState<TinyEvent> reducedImpression;

  /** {@code reducedImpression} should be replaced after this timestamp (or if this is null). */
  @VisibleForTesting transient ValueState<Long> replaceReducedImpressionAfter;

  /**
   * Keeps track of out of order actions for the first impression. Just keep in a {@code ListState}.
   * This should be small because we require an inner join between impressions and actions. If it
   * grows large, we probably have a bug in the pipeline.
   */
  @VisibleForTesting transient ListState<TinyEvent> outOfOrderActions;

  /** The latest cleanup timer. When this timer is hit, clear out all state. */
  @VisibleForTesting transient ValueState<Long> latestCleanupTime;
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    ValueStateDescriptor<TinyEvent> reducedImpressionDescriptor =
        new ValueStateDescriptor<>("root-impression", TypeInformation.of(TinyEvent.class));
    ...
    StateTtlConfig ttlConfig =
        StateTtlConfig.newBuilder(Time.milliseconds(TMP_TTL_MULTIPLIER * ttl.toMillis()))
    ...
    ValueStateDescriptor<Long> replaceReducedImpressionAfterDescriptor =
        new ValueStateDescriptor<>("replace-reduced-impression-after", Types.LONG);
    replaceReducedImpressionAfterDescriptor.enableTimeToLive(ttlConfig);
    replaceReducedImpressionAfter =
        getRuntimeContext().getState(replaceReducedImpressionAfterDescriptor);

    ValueStateDescriptor<Long> latestCleanupTimeDescriptor =
        new ValueStateDescriptor<>("latest-cleanup-time", Types.LONG);
    latestCleanupTime = getRuntimeContext().getState(latestCleanupTimeDescriptor);

    ListStateDescriptor<TinyEvent> outOfOrderActionsDescriptor =
        new ListStateDescriptor<>("out-of-order-actions", TypeInformation.of(TinyEvent.class));
  ...
  @Override
  public void processElement(TinyEvent input, Context context, Collector<TinyEvent> collector)
      throws Exception {
    ...
    if (input.getActionId().isEmpty()) {
      ...
      if (reducedImpression == null || isAfterReplacementTime(context.timestamp())) {
        ...
        // TODO - add a wrapper around the TinyEvent that contains the ID of
        // reducedImpression.impressionId.
  ...
  /**
   * Register clean-up timer. Might also update {@code latestCleanupTime} which is used to clean out
   * the last state.
   */
  ...
  private void processAnyOutOfOrderActions(TinyEvent input, Collector<TinyEvent> collector)
      throws Exception {
    ...
    if (oufOfOrderActionIterable.iterator().hasNext()) {
      ...
      oufOfOrderActionIterable.forEach(
          action -> {
  ...
  public void onTimer(long timestamp, OnTimerContext ctx, Collector<TinyEvent> out)
    ...
    // Process out-of-order actions.  This should not happen unless we have a joined-action without
    // a joined-impression
    // (so a bug, edge condition or new feature development that misses this code).  This code
    // outputs these
    // impression-less actions.
    //
    // Other considered short-term options:
    // 1. Kill the job.  Long-term, we do not want to blow up production.  It might make sense to
    // have these explode in dev environments.
    //    TODO(PRO-1627) - introduce a flag setup that lets us log errors in production but explode
    // in canary and dev.
    // 2. Drop the action.  This is similar to outputting since full details are not kept around
    // very long.
    //
    ...
    for (TinyEvent action : outOfOrderActions.get()) {
      if (timestamp >= toCleanupTime(action.getLogTimestamp())) {
        LOGGER.error(
            "Encountered unexpected out-of-order tiny action that did not have a corresponding tiny impression, action={}",
            action);
        out.collect(action);
      } else {
    ...
    if (timestamp == latestCleanupTime.value()) {
      ...
      if (!newList.isEmpty()) {
        LOGGER.error(
            "This should never happen.  During final clean-up, we encountered existing outOfOrderActions without a clean-up timer.");
      }
      clearAllState();
    }
  }
   ...
   *
   * <p>This usually will usually create bugs (since that time might not match the event's Flink
   * time). In this case, it's okay since the cleanup code is to handle a corner case which should
   * not happen. If it happens, we'll have bigger bugs.
   */
