...
/**
 * This iterator produces fake data {@code LogRequests} with delays to mimic real traffic.  This is not thread safe.
 *
 * <ul>
 *     <li>
 *         A heap is used to manage future {@code LogRequests}.  The first item might be in {@code nextState} variable.
 *         This allows us to delay child {@code LogRequests}.
 *     </li>
 *     <li>
 *         The heap values contains an external {@code LogRequest} (returned to clients) and an optional
 *         {@code Runnable}.  The {@code Runnable} is executed when we want to add more state to the heap.
 *         The {@code Runnable} can get executed while calling {@code hasNext()} or {@code peekTime}.
 *     </li>
 * </ul>
 *
 * <p>Consumers should call {@link #hasNext()} and {@link #peekTime()} to control this LogRequestIterator.  When the time is
 * passed, then {@link #next()} should be called.
 *
 * Design notes for {@code addAbcToHeap} methods
 * <ul>
 *     <li>
 *         The methods are designed to be simple.  Instead of having an explicit state machine, the methods are
 *         responsible for figuring out the next state.  They can look at options and other parameters
 *         to influence the next states for the generator.  A single method can create multiple future outputs
 *         for this iterator.
 *     </li>
 *     <li>
 *         When adding or modifying, assume the methods can be executed when peeking.
 *     </li>
 *     <li>
 *         Group the {@code addAbcToHeap} methods together.
 *     </li>
 *     <li>
 *         Timing logic is broken.  The caller waits using {@code peekTime()}.  The actual timing
 *         on the records will look to see what {@code now()} is.  These might not be the same.
 *         The timing can now start executing earlier since those methods get called while peeking.
 *
 *         TODO - fix the time logic.  E.g. change the {@code addAbcToHeap} methods to pass through the times.
 *         E.g. change the background runnables to have a slightly later timestamp (+1ms).
 *     </li>
 *     <li>
 *         This {@code Runnable} design would make it hard to distribute state of the {@code LogRequestIterator}.
 *         That's not important right now.  The {@code Runnable} design makes it easier to delay execution of
 *         derived state.
 *     </li>
 * </ul>
 */
public final class LogRequestIterator implements Iterator<LogRequest> {
    ...
    */
    private final MinMaxPriorityQueue<State> stateHeap = MinMaxPriorityQueue.<State>orderedBy(
            Ordering.<State>compound(ImmutableList.of(
                    Ordering.natural().onResultOf(state -> state.time()),
                    Ordering.natural().onResultOf(state -> state.index())))).create();
    private int nextStateIndex;
    // The Cart state is not in sync with stateHeap.
    // TODO - synchronize it.  It'd probably mean keeping track of updates with timestamps.  The complexity isn't
    // worth it right now.
    ...
    /**
     * Processes the earliest heap states up to {@code stateHeap} is {@code LogRequest}.
     * This will call {@code runnable().run()} on each state that is processed.
     */
    ...
    /**
     * Used to delay events to simulate delayed event behavior.  Uses
     * delayMultiplier to allow for randomness and scaling.
     */
    ...
    public LogRequest next() {
        processStatesUntilNextStateIsLogRequest();
        Preconditions.checkState(nextState != null, "No more LogRequests.  Make sure to call hasNext first.");
        nextState.runnable().run();
    ...
    private void addCohortMembershipsToHeap(LogRequest request, String userId) {
        ...
        LogRequest external = toCommonFieldsBuilder(request)
                .setTiming(createTiming(eventApiTimestamp))
                .addCohortMembership(
    ...
    private void addSessionsToHeap(LogRequest request, String userId, String logUserId) {
        IntStream.range(0, options.sessionsPerUser()).forEach(i -> addSessionToHeap(request, userId, logUserId));
    }

    private void addSessionToHeap(LogRequest request, String userId, String logUserId) {
        Instant now = options.now();
        Instant eventApiTimestamp = options.eventApiTimestamp();
        LogRequest.Builder builder = toCommonFieldsBuilder(request)
                .setUserInfo(UserInfo.newBuilder()
                        .setUserId(userId)
                        .setLogUserId(logUserId))
                .setTiming(createTiming(eventApiTimestamp));
        final String sessionId = options.sessionUuid();
        builder.addSessionProfile(options.sessionProfileTransform().apply(
                SessionProfile.newBuilder()
                        .setSessionId(sessionId)
                        .setTiming(createTiming(eventApiTimestamp))));
        builder.addSession(options.sessionTransform().apply(
                Session.newBuilder()
                        .setSessionId(sessionId)
    ...
    private void addViewsToHeap(LogRequest request, String sessionId, ImmutableList<FakeInsertionSurface> requestInsertionSurfaces) {
        IntStream.range(0, options.viewsPerSession()).forEach(i -> addViewToHeap(
                request,
                sessionId,
                i,
                "",
                ...
                ImmutableList.of(ContentDBFilter.whereFieldIs(Content.TYPE, requestInsertionSurfaces.get(0).contentType().name())),
    ...
    private void addViewToHeap(
        ...
        Instant viewEventApiTimestamp = delay(eventApiTimestamp, Duration.ofSeconds(index * options.delayBetweenViewInSeconds()));
        String viewId = options.viewUuid();
        LogRequest logRequest = toCommonFieldsBuilder(request)
                .setTiming(createTiming(viewEventApiTimestamp))
                .addView(options.viewTransform().apply(
                        View.newBuilder()
                                .setViewId(viewId)
                                .setSessionId(sessionId)
                                .setTiming(createTiming(viewEventApiTimestamp))
        ...
        if (addDeliveryLogs) {
            addToHeap(viewNow, () -> {
                ...
                contents = contents.subList(0, Math.min(contents.size(), options.responseInsertionsPerRequest()));
    ...
    private void addAutoViewsToHeap(LogRequest request, String sessionId) {
        IntStream.range(0, options.autoViewsPerSession()).forEach(i -> addAutoViewToHeap(request, sessionId, i));
    }

    private void addAutoViewToHeap(LogRequest request, String sessionId, int index) {
        Instant now = options.now();
        Instant eventApiTimestamp = options.eventApiTimestamp();
        Instant autoViewNow = delay(now, Duration.ofSeconds(index * options.delayBetweenAutoViewInSeconds()));
        Instant autoViewEventApiTimestamp = delay(eventApiTimestamp, Duration.ofSeconds(index * options.delayBetweenAutoViewInSeconds()));
        String autoViewId = options.autoViewUuid();
        LogRequest logRequest = toCommonFieldsBuilder(request)
                .setTiming(createTiming(autoViewEventApiTimestamp))
                .addAutoView(options.autoViewTransform().apply(
                        AutoView.newBuilder()
    ...
    private void addDeliveryLogsToHeap(LogRequest logRequest, String viewId, ImmutableList<? extends Content> contents, ImmutableList<FakeInsertionSurface> requestInsertionSurfaces) {
        IntStream.range(0, options.requestsPerViews()).forEach(i -> addDeliveryLogToHeap(logRequest, viewId, contents, requestInsertionSurfaces, i));
    }

    // TODO - support SDK fallback SDKs.
    // TODO - change the response insertions in shadow traffic so they are different than SDK response insertions.
    private void addDeliveryLogToHeap(LogRequest logRequest, String viewId, ImmutableList<? extends Content> contents, ImmutableList<FakeInsertionSurface> requestInsertionSurfaces, int index) {
        Instant now = options.now();
        Instant eventApiTimestamp = options.eventApiTimestamp();
        List<InsertedContent> insertedContents = newInsertedContents(contents);
        DeliveryLog baseDeliveryLog = createBaseDeliveryLog(logRequest, now, viewId, insertedContents, requestInsertionSurfaces.get(0).contentType());
        ...
        baseDeliveryLog = baseDeliveryLog.toBuilder()
                .setRequest(baseDeliveryLog.getRequest().toBuilder().setClientRequestId(clientRequestId))
                .build();
        boolean generateMiniSdkDeliveryLog = matchesRateOption(options.miniSdkRate(), baseDeliveryLog.getRequest().getRequestId(), "addDeliveryLogsToHeap");
        ...
        DeliveryLog deliveryLog = options.deliveryLogTransform().apply(undecoratedDeliveryLog.toBuilder()).build();
        LogRequest.Builder baseBuilder = toCommonFieldsBuilder(logRequest)
                .setTiming(createTiming(eventApiTimestamp));
        Instant requestInstant = delay(now, Duration.ofMillis(500 * (index + 1)));

        // Tmp code.
        final LogRequest deliveryLogLogRequest = baseBuilder.clone().addDeliveryLog(deliveryLog).build();
        ...
        addToHeap(requestInstant,
            () -> addImpressionsToHeap(deliveryLogLogRequest, deliveryLog.getRequest().getRequestId(), copyOf(insertedContents), requestInsertionSurfaces));

        boolean generateShadowTrafficDeliveryLog = generateMiniSdkDeliveryLog
                && matchesRateOption(options.shadowTrafficRate(), baseDeliveryLog.getRequest().getRequestId(), "addDeliveryLogsToHeap2");
        if (generateShadowTrafficDeliveryLog) {
            // We need a new requestId so create a new base.
            DeliveryLog baseShadowTrafficDeliveryLog = baseDeliveryLog.toBuilder()
                    .setRequest(baseDeliveryLog.getRequest().toBuilder()
                            .setRequestId(options.requestUuid())
                            .setClientRequestId(clientRequestId))
                    .build();
            DeliveryLog shadowTrafficDeliveryLog = options.deliveryLogTransform().apply(
                    createShadowDeliveryApiDeliveryLog(baseShadowTrafficDeliveryLog).toBuilder())
                    .build();
    ...
    /**
     * Get the response insertions and create impressions for insertionImpressionRate of them.
     * Done as a batch since Impressions are usually logged in batches.
     */
    private void addImpressionsToHeap(LogRequest logRequest, String requestId, ImmutableList<InsertedContent> insertedContents, ImmutableList<FakeInsertionSurface> requestInsertionSurfaces) {
        // If we should log redundant impressions, add all of them to the heap with different delays.
        boolean redundantImpressions = matchesRateOption(options.redundantImpressionRate(), requestId, "addImpressionsToHeap1");
        int numRedundantImpressions = redundantImpressions ? options.maxRedundantImpressionsPerDeliveryLog() : 1;
        int maxImpressions = fromRateToCount(options.insertionImpressedRate(), insertedContents.size(), requestId, "addImpressionsToHeap2");
        ...
        for (int i = 0; i < numRedundantImpressions; i++) {
            ...
            List<ImpressedContent> impressedContents = newImpressedContent(insertedContents.subList(0, maxImpressions));
            addImpressionsToHeap(logRequest, copyOf(impressedContents), Duration.ofSeconds(2 + i * 2), createActions, requestInsertionSurfaces);
        }
    }
     ...
     */
    private void addImpressionsToHeap(LogRequest logRequest, ImmutableList<ImpressedContent> impressedContents, Duration delay, boolean createActions, ImmutableList<FakeInsertionSurface> requestInsertionSurfaces) {
        ...
        Instant now = options.now();
        Instant eventApiTimestamp = options.eventApiTimestamp();
        LogRequest.Builder builder = toCommonFieldsBuilder(logRequest)
                .setTiming(createTiming(eventApiTimestamp));
        // Add the top insertions as impressions to the LogRequest.
        for (ImpressedContent impressedContent : impressedContents) {
            Impression.Builder impression = Impression.newBuilder()
        ...
        boolean isMissingEvent = builder.getImpressionCount() > 0 && isMissingEvent(options.missingImpressionRate(), firstImpressionId, "addImpressionsToHeap1");
        LogRequest external = builder.build();
        if (!isMissingEvent) {
            addToHeap(impressionInstant, external);
        }
        if (createActions) {
            addToHeap(impressionInstant, () -> {
                int maxNavigates = fromRateToCount(options.impressionNavigateRate(), impressedContents.size(), firstImpressionId, "addImpressionsToHeap2");
                List<ImpressedContent> navigateContents = impressedContents.subList(0, maxNavigates);
                addNavigatesToHeap(external, copyOf(navigateContents), copyOf(requestInsertionSurfaces));
            });
        }
    }
     ...
     */
    private void addNavigatesToHeap(LogRequest request, List<ImpressedContent> navigateContents, List<FakeInsertionSurface> requestInsertionSurfaces) {
        ...
        addToHeap(actionInstant, () -> {
            String firstImpressionId = navigateContents.get(0).impressionId();
            if (requestInsertionSurfaces.size() == 1) {
                int maxNavigates = fromRateToCount(options.navigateCheckoutRate(), navigateContents.size(), firstImpressionId, "addNavigatesToHeap");
                ...
                List<FakeInsertionSurface> nextRequestInsertionSurfaces = requestInsertionSurfaces.subList(1, requestInsertionSurfaces.size());
                for (int i = 0; i < navigateContents.size(); i++) {
                    ...
                    addViewToHeap(
                            ...
                            ImmutableList.of(
                                    ...
                                    ContentDBFilter.whereFieldIs(requestInsertionSurfaces.get(0).contentType().id, contentId),
                                    // AND matching the desired type.
                                    ContentDBFilter.whereFieldIs(Content.TYPE, nextRequestInsertionSurfaces.get(0).contentType().name())),
    ...
    private void addAddToCartActions(LogRequest request, List<ImpressedContent> navigateContents, Instant eventApiTimestamp, LogRequest.Builder builder) {
        String randomSeed = navigateContents.get(0).impressionId();
        int numAddToCart = fromRateToCount(options.navigateAddToCartRate(), navigateContents.size(), randomSeed, "addToCart");
        ...
        for (ImpressedContent addToCartContent : addToCartContents) {
            ...
            builder.addAction(Action.newBuilder()
                    .setActionId(options.actionUuid())
            ...
            // Add cart
            Cart cart = logUserIdToCart.getOrDefault(request.getUserInfo().getLogUserId(), Cart.getDefaultInstance());
            ...
            cart = cart.toBuilder()
                    .addContents(CartContent.newBuilder()
                            ...
                            .setPricePerUnit(Money.newBuilder()
    ...
    private void addCheckoutsToHeap(LogRequest request, List<ImpressedContent> checkoutContents) {
        ...
        addToHeap(actionInstant, () -> {
            ...
            int maxPurchases = fromRateToCount(options.checkoutPurchaseRate(), checkoutContents.size(), firstImpressionId, "addCheckoutsToHeap");
    ...
    private void addDiagnosticsToHeap(LogRequest request) {
        ...
        Diagnostics.Builder diagnostics = Diagnostics.newBuilder()
                .setTiming(createTiming(eventApiTimestamp))
                .setMobileDiagnostics(MobileDiagnostics.newBuilder()
                        .setDeviceIdentifier("abcd-efgh"));
    ...
    private DeliveryLog createBaseDeliveryLog(
            LogRequest logRequest, Instant eventApiTimestamp, String viewId, List<InsertedContent> contents, ContentType requestContentType) {
        ...
        for (int position = 0; position < contents.size(); position++) {
            ...
            response.addInsertion(createResponseInsertion(content, requestContentType, requestId, eventApiTimestamp, position));
        }
    ...
    private void addInsertionMatrix(List<InsertedContent> contents, Request.Builder request) {
        ...
        for (int position = 0; position < contents.size(); position++) {
            Map<String, Value> matrixRow = createRequestInsertionMatrixRow(contents.get(position).content());
    ...
    private ImmutableList<FakeInsertionSurface> getRequestInsertionSurfaces() {
        ImmutableList.Builder<FakeInsertionSurface> builder = ImmutableList.<FakeInsertionSurface>builder()
                .add(options.requestInsertionSurface0());
        options.requestInsertionSurface1().ifPresent(surface -> builder.add(surface));
        return builder.build();
    }

    /**
     * Creates a Production Delivery API DeliveryLog from a baseDeliveryLog.
     * It adds execution insertions with a fake feature.
     */
    ...
    /**
     * Creates a Shadow Traffic Delivery API DeliveryLog from a baseDeliveryLog.
     * It adds execution insertions with a fake feature.
     */
    ...
    /**
     * Creates a Delivery API DeliveryLog from a baseDeliveryLog.
     * It adds execution insertions with a fake feature.  Supports custom TrafficType arg.
     */
    private DeliveryLog createDeliveryApiDeliveryLog(
        ...
        DeliveryExecution.Builder executionBuilder = DeliveryExecution.newBuilder()
        ...
        for (Insertion baseExecutionInsertion : toBaseExecutionInsertions.apply(baseDeliveryLog)) {
            ...
            Insertion executionInsertion = baseExecutionInsertion.toBuilder()
                    .setFeatureStage(FeatureStage.newBuilder()
                            .setFeatures(Features.newBuilder()
                                    .putNumeric(100, 0.1f * (contentId % 11))
                            ))
                    .build();
    ...
    private Request.Builder createRequest(String viewId, Instant eventApiTimestamp, UserInfo userInfo) {
    ...
    private Insertion.Builder createResponseInsertion(InsertedContent content, ContentType requestContentType, String requestId, Instant eventApiTimestamp, int position) {
        return options.responseInsertionTransform().apply(
    ...
    /**
     * Returns a fraction of {@code size} by using {@code rate}.  Uses {@code id}
     * to deterministically do ceiling or floor at a {@code rate}.
     */
    ...
    private Insertion.Builder createRequestInsertion(Content content) {
        ...
        content.requestFields().entrySet()
                .stream()
    ...
    private Map<String, Value> createRequestInsertionMatrixRow(Content content) {
        ...
        content.requestFields().entrySet().forEach(entry -> {
            builder.put(entry.getKey(), Value.newBuilder().setStringValue(entry.getValue()).build());
        });
