...
/**
 * Predicate that is used to keep (true) or filter out (false) flat events that have a sparseInt matching one of the
 * following.
 *
 * <p>This is inlined for now in the code to avoid having a different DB.</p>
 *
 * <p>To calculate the hash, use go-hashlib and modifying a unit test to find the sparseInt.</p>
 *
 * <p>Known issues</p>
 * <ul>
 *     <li>This relies on having an API Execution Insertion.  This does not catch the case where we are missing
 *     API Execution Insertions (e.g. not calling shadow traffic or we have a bug in the join).
 *     For now, Dan thinks this is okay.  When we want to support other cases, we either need to join
 *     User DB in Flink our get them to specify UserInfo.ignore_usage.</li>
 * </ul>
 */
public final class BuyerPredicate<T> implements SerializablePredicate<T> {
    ...
    public BuyerPredicate(List<Long> nonBuyerUserSparseHashes, SerializableFunction<T, Insertion> toExecutionInsertion) {
