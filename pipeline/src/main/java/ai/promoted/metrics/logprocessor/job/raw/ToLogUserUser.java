...
class ToLogUserUser implements FlatMapFunction<LogRequest, LogUserUser> {
    ...
    public void flatMap(LogRequest logRequest, Collector<LogUserUser> collector) throws Exception {
        ...
        logRequest.getUserList().forEach(user -> output(platformId, eventApiTimestamp, user, collector));
        logRequest.getCohortMembershipList().forEach(cohortMembership -> output(platformId, eventApiTimestamp, cohortMembership, collector));
        logRequest.getViewList().forEach(view -> output(platformId, eventApiTimestamp, view, collector));
        logRequest.getAutoViewList().forEach(autoView -> output(platformId, eventApiTimestamp, autoView, collector));
        logRequest.getDeliveryLogList().forEach(deliveryLog -> output(platformId, eventApiTimestamp, deliveryLog, collector));
        logRequest.getImpressionList().forEach(impression -> output(platformId, eventApiTimestamp, impression, collector));
        logRequest.getActionList().forEach(action -> output(platformId, eventApiTimestamp, action, collector));
        logRequest.getDiagnosticsList().forEach(diagnostics -> output(platformId, eventApiTimestamp, diagnostics, collector));
    }

    private void output(long platformId, long eventApiTimestamp, User user, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, CohortMembership cohortMembership, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, View view, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, AutoView autoView, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, DeliveryLog deliveryLog, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, Impression impression, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, Action action, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, Diagnostics diagnostics, Collector<LogUserUser> collector) {
    ...
    private void output(long platformId, long eventApiTimestamp, UserInfo userInfo, Collector<LogUserUser> collector) {
        if (!userInfo.getLogUserId().isEmpty() && !userInfo.getUserId().isEmpty()) {
            collector.collect(LogUserUser.newBuilder()
