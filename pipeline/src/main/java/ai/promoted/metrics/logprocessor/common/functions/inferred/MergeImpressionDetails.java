...
/**
 * Takes tiny impressions (from Inferred References) and fills in details using the stream of UnionEvent.
 */
public class MergeImpressionDetails extends AbstractMergeDetails {
    ...
    @Override
    public void processElement1(UnionEvent input, Context ctx, Collector<JoinedEvent> out) throws Exception {
        // TODO - optimize the full records being stored in maps.  We can strip out fields we'll eventually clear.
    ...
    @Override
    protected void addIncompleteTimers(TinyEvent rhs, Context ctx, EnumSet<AbstractMergeDetails.MissingEvent> missing, long timerTimestamp) throws Exception {
        for (MissingEvent missingEvent : missing) {
            switch (missingEvent) {
                case VIEW:
                    addIncompleteTimers(viewMerger.viewIdToIncompleteEventTimers, rhs.getViewId(), timerTimestamp);
                    break;
                case DELIERY_LOG:
                    addIncompleteTimers(deliveryLogMerger.requestIdToIncompleteEventTimers, rhs.getRequestId(), timerTimestamp);
                    break;
                case IMPRESSION:
                    addIncompleteTimers(impressionMerger.impressionIdToIncompleteEventTimers, rhs.getImpressionId(), timerTimestamp);
                    break;
                default:
    ...
    @Override
    protected String getAllRelatedStateString(String viewId, String requestId, String impressionId, String actionId) throws Exception {
    ...
    @Override
    protected EnumSet<MissingEvent> fillInFull(JoinedEvent.Builder builder, TinyEvent rhs, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger) throws Exception {
    ...
    final class ViewMerger extends BaseMerger<View> {
        ...
        public void open(RuntimeContext runtimeContext) throws Exception {
            idToView = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-view", String.class, View.class));
            viewIdToIncompleteEventTimers = runtimeContext.getMapState(
                    // TODO(PRO-1683) - add caches back in.
                    new MapStateDescriptor<>("view-id-to-incomplete-event-timers", Types.STRING, Types.LIST(Types.LONG)));
        }
        ...
        @Override
        protected void tryProcessIncompleteEvents(View view, boolean matchesDebugId, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger, Collector<JoinedEvent> out) throws Exception {
            tryProcessIncompleteEvents(view.getViewId(), matchesDebugId, viewIdToIncompleteEventTimers, errorLogger, out);
        }
        ...
        protected String getDebugStateString(String id) throws Exception {
            ...
            return String.format("ViewMerger{idToView[%s]=%s, viewIdToIncompleteEventTimers[%s]=%s}", id, view, id, incompleteEventTimers);
        }

        /** Returns true iff all of the requested fields are included. */
        public boolean fillInFull(JoinedEvent.Builder builder, TinyEvent rhs, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger) throws Exception {
        ...
        public void cleanup(long timestamp) throws Exception {
            cleanupDetailsState(idToView, timestamp, (View view) -> view.getTiming().getLogTimestamp(), window, "View", debugIds::matchesViewId);
    ...
    final class DeliveryLogMerger extends BaseMerger<CombinedDeliveryLog> {
        ...
        public void open(RuntimeContext runtimeContext) throws Exception {
            idToCombinedDeliveryLog = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-combineddeliverylog", String.class, CombinedDeliveryLog.class));
            idToInsertion = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-insertion", String.class, InsertionBundle.class));
            requestIdToIncompleteEventTimers = runtimeContext.getMapState(
                    // TODO(PRO-1683) - add caches back in.
                    new MapStateDescriptor<>("request-id-to-incomplete-event-timers", Types.STRING, Types.LIST(Types.LONG)));
        }
        ...
        protected void addFull(CombinedDeliveryLog input) throws Exception {
            ...
            for (InsertionIdAndBundle idAndBundle : insertions) {
                ...
                Preconditions.checkArgument(!responseInsertionId.isEmpty(),
                        "Response Insertion should have insertionId, combinedDeliveryLog=%s", input);
                idToInsertion.put(responseInsertionId, idAndBundle.bundle());
            }
        }

        @Override
        protected void tryProcessIncompleteEvents(CombinedDeliveryLog deliveryLog, boolean matchesDebugId, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger, Collector<JoinedEvent> out) throws Exception {
            // A little redundant work.
            String requestId = DeliveryLogUtil.getRequestId(deliveryLog);
            tryProcessIncompleteEvents(requestId, matchesDebugId, requestIdToIncompleteEventTimers, errorLogger, out);
        }
        ...
        protected String getDebugStateString(String id) throws Exception {
            ...
            return String.format("DeliveryLogMerger{idToCombinedDeliveryLog[%s]=%s, idToInsertion[%s]=%s, viewIdToIncompleteEventTimers[%s]=%s}",
        ...
        public boolean fillInFull(JoinedEvent.Builder builder, TinyEvent rhs, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger) throws Exception {
            ...
            if (!rhs.getRequestId().isEmpty()) {
                ...
                if (combinedDeliveryLog != null) {
                    ...
                    if (combinedDeliveryLog.hasApi() && combinedDeliveryLog.hasSdk()) {
                        ...
                        // Most of the Request fields are the same.  We'll keep fields that we expect to be different in HiddenApiRequest.
                        builder.setHiddenApiRequest(toHiddenApiRequest(combinedDeliveryLog.getApi()));
                    }
                    FlatUtil.setFlatResponse(builder, DeliveryLogUtil.getDeliveryLog(combinedDeliveryLog).getResponse());
            ...
            if (insertion != null) {
                if (insertion.hasRequestInsertion()) {
                    FlatUtil.setFlatRequestInsertion(builder, insertion.getRequestInsertion(), rhs.getLogTimestamp(), errorLogger);
                }
                if (insertion.hasResponseInsertion()) {
                    FlatUtil.setFlatResponseInsertion(builder, insertion.getResponseInsertion(), rhs.getLogTimestamp(), errorLogger);
                }
                if (insertion.hasApiExecutionInsertion()) {
                    FlatUtil.setFlatApiExecutionInsertion(builder, insertion.getApiExecutionInsertion(), rhs.getLogTimestamp(), errorLogger);
                }
            } else {
                allMatched = false;
            }
        ...
        public void cleanup(long timestamp) throws Exception {
            cleanupDetailsState(idToCombinedDeliveryLog, timestamp, (CombinedDeliveryLog deliveryLog) -> getRequest(deliveryLog).getTiming().getLogTimestamp(),
                    window, "CombinedDeliveryLog", debugIds::matchesRequestId);
            cleanupDetailsState(idToInsertion, timestamp, InsertionBundle::getLogTimestamp, window, "Insertion", debugIds::matchesInsertionId);
            cleanupOldTimers(requestIdToIncompleteEventTimers, timestamp, "Request");
        }

        private CombinedDeliveryLog.Builder stripInsertionAndUnusedFields(CombinedDeliveryLog combinedDeliveryLog) {
            CombinedDeliveryLog.Builder builder = combinedDeliveryLog.toBuilder();
            if (builder.hasSdk() && builder.hasApi()) {
                // If the JoinedEvent has both SDK and API Requests, the SDK Response was used.
                // Most of the Request fields are the same.  We'll keep fields that we expect to be different in HiddenApiRequest.
                builder.getApiBuilder()
                        .clearResponse()
        ...
        private List<InsertionIdAndBundle> toInsertionBundles(CombinedDeliveryLog combinedDeliveryLog) {
            ...
            Map<String, Insertion> contentIdToRequestInsertion = deliveryLog.getRequest().getInsertionList().stream()
                    .collect(Collectors.toMap(
                            Insertion::getContentId,
                            Function.identity(),
                            (insertion1, insertion2) -> {
                                // If we see duplicate contentIds, we need to verify how duplicate contentIds work.
                                LOGGER.error("Multiple request insertions with the same contentId, {}, found on DeliveryLog={}", insertion1.getContentId(), deliveryLog);
                                // Just take the first insertion for now.
                                return insertion1;
                            }));
            Map<String, Insertion> insertionIdToApiExecutionInsertion = combinedDeliveryLog.getApi().getExecution().getExecutionInsertionList().stream()
                    ...
                    .collect(Collectors.toMap(
                            ...
                            Function.identity(),
                            (insertion1, insertion2) -> {
                                LOGGER.error("Multiple execution insertions with the same insertionId, {}, found on combinedDeliveryLog.getApi()={}", insertion1.getInsertionId(), combinedDeliveryLog.getApi());
                                // Just take the first insertion for now.
                                return insertion1;
                            }));
            Map<String, Insertion> contentIdToApiExecutionInsertion = combinedDeliveryLog.getApi().getExecution().getExecutionInsertionList().stream()
                    .collect(Collectors.toMap(
                            Insertion::getContentId,
                            Function.identity(),
                            (insertion1, insertion2) -> {
                                // Just take the first insertion for now.  It's more likely to get an impression.
            ...
            return deliveryLog.getResponse().getInsertionList().stream().map(responseInsertion -> {
                InsertionBundle.Builder builder = InsertionBundle.newBuilder()
                        .setLogTimestamp(logTimestamp);
                builder.setResponseInsertion(FlatUtil.clearRedundantFlatInsertionFields(responseInsertion.toBuilder()));
                ...
                if (requestInsertion != null) {
                    builder.setRequestInsertion(FlatUtil.clearRedundantFlatInsertionFields(requestInsertion.toBuilder()));
                }
                String insertionId = responseInsertion.getInsertionId();
                Insertion apiExecutionInsertion = insertionIdToApiExecutionInsertion.get(insertionId);
                if (apiExecutionInsertion == null) {
                    apiExecutionInsertion = contentIdToApiExecutionInsertion.get(contentId);
                }
                if (apiExecutionInsertion != null) {
                    builder.setApiExecutionInsertion(FlatUtil.clearRedundantFlatInsertionFields(apiExecutionInsertion.toBuilder()));
                }
        ...
        private void stripInsertionBundle(InsertionBundle.Builder builder) {
            ...
            if (builder.hasRequestInsertion()) {
                builder.getRequestInsertionBuilder()
                        .clearInsertionId()
                        .clearContentId();
            }
            if (builder.hasApiExecutionInsertion()) {
                builder.getApiExecutionInsertionBuilder()
                        .clearInsertionId()
                        .clearContentId();
            }
            if (builder.hasResponseInsertion()) {
                builder.getResponseInsertionBuilder()
                        .clearInsertionId();
            }
        }
    ...
    final class ImpressionMerger extends BaseMerger<Impression> {
        ...
        @VisibleForTesting
        MapState<String, Impression> idToImpression;
        ...
        public void open(RuntimeContext runtimeContext) throws Exception {
            idToImpression = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-impression", String.class, Impression.class));
            impressionIdToIncompleteEventTimers = runtimeContext.getMapState(
                    // TODO(PRO-1683) - add caches back in.
                    new MapStateDescriptor<>("impression-id-to-incomplete-event-timers", Types.STRING, Types.LIST(Types.LONG)));
        }
        ...
        @Override
        protected void tryProcessIncompleteEvents(Impression impression, boolean matchesDebugId, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger, Collector<JoinedEvent> out) throws Exception {
            tryProcessIncompleteEvents(impression.getImpressionId(), matchesDebugId, impressionIdToIncompleteEventTimers, errorLogger, out);
        }

        @Override
        protected String getDebugStateString(Impression impression) throws Exception {
            return getAllRelatedStateString(impression.getViewId(), impression.getRequestId(), impression.getImpressionId(), "");
        }
        ...
        protected String getDebugStateString(String id) throws Exception {
            ...
            return String.format("ImpressionMerger{idToImpression[%s]=%s, impressionIdToIncompleteEventTimers[%s]=%s}", id, impression, id, incompleteEventTimers);
        }
        ...
        public void cleanup(long timestamp) throws Exception {
            cleanupDetailsState(idToImpression, timestamp, (Impression impression) -> impression.getTiming().getLogTimestamp(),
                    window, "Impression", debugIds::matchesImpressionId);
