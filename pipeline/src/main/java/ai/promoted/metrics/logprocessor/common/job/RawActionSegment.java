...
/**
 * A FlinkSegment that provides actions as a source.  It's a separate FlinkSegment
 * to isolate the flags to just jobs that use the deduplication source.
 */
public class RawActionSegment implements FlinkSegment {

    @CommandLine.Option(names = {"--keepFirstActionDuration"}, defaultValue = "P1D", description = "The duration to keep track of recent Actions.  This is used to de-duplicate raw inputs.  Default=P1D to match flat_response_insertion join window.  Java8 Duration parse format.")
    ...
    public DataStream<Action> getDeduplicatedAction(DataStream<Action> action) {
        return baseFlinkJob.add(
                action.keyBy(KeyUtil.actionKeySelector)
                        .process(new KeepFirstRow<>("action", keepFirstActionDuration),
