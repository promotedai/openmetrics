...
/**
 * Serializes Protobufs in a record format that supports Kafka's Java client.
 * This is used in fake data generator.  Flink uses the key and value serialization schemas directly.
 */
public class ProtoKafkaSerializationSchema<T extends GeneratedMessageV3> {
    ...
    public ProtoKafkaSerializationSchema(
            String topic,
            PlatformLogUserKeySerializationSchema<T> keySerializationSchema) {
        this.topic = topic;
