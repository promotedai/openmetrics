...
public class CombineDeliveryLog
  ...
  @Override
  public void onTimer(long timestamp, OnTimerContext ctx, Collector<CombinedDeliveryLog> out)
      throws Exception {
  ...
  public void open(Configuration config) throws Exception {
    super.open(config);
    clientRequestIdToCombinedDeliveryLog =
        getRuntimeContext()
            .getMapState(
    ...
    timerToClientRequestId =
        getRuntimeContext()
            .getMapState(
                // TODO(PRO-1683) - add caches back in.
                new MapStateDescriptor<>(
                    "timer-to-client-request-id", Types.LONG, Types.LIST(Types.STRING)));
  }

  static CombinedDeliveryLog.Builder mergeDeliveryLog(
      CombinedDeliveryLog.Builder builder, DeliveryLog deliveryLog) {
    switch (deliveryLog.getExecution().getExecutionServer()) {
      case SDK:
        ...
        if (!builder.hasSdk()
            || getTrafficPriority(deliveryLog) >= getTrafficPriority(builder.getSdk())) {
          builder.setSdk(deliveryLog);
        }
        break;
      case API:
      case SIMPLE_API:
        // TODO - after enough time has passed, we can change UNKNOWN_EXECUTION_SERVER to behave
        // differently.
      ...
      default:
        throw new UnsupportedOperationException(
            "Unsupported ExecutionServer=" + deliveryLog.getExecution().getExecutionServer());
    }
   ...
   *
   * <p>SDKs do not generating pagingIds for SDK DeliveryLogs. Our duplicate content ID checks would
   * benefit from having paging IDs on joined events. Short-term, we can either fix it here or in
   * merge details stage.
   *
   * <p>This is a hack. It's better than nothing. The longer-term solution depends on how much we
   * want to support in the SDK directly. E.g. do we actually implement a full paging id solution in
   * each SDK? Dan would prefer that we do not do this. We want to keep the SDK layer small.
   */
  private static CombinedDeliveryLog.Builder fixSdkResponse(CombinedDeliveryLog.Builder builder) {
    if (builder.hasApi()
      ...
      builder
          ...
          .setPagingId(builder.getApi().getResponse().getPagingInfo().getPagingId());
    }
    return builder;
  }
}
