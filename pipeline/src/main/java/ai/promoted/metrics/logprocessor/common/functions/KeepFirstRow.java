...
public class KeepFirstRow<K, T> extends KeyedProcessFunction<K, T, T> {
  ...
  @Override
  public void processElement(T value, KeyedProcessFunction<K, T, T>.Context ctx, Collector<T> out)
      throws Exception {
    // Since value can be null.
    if (alreadyOutputted.value() != Boolean.TRUE) {
      alreadyOutputted.update(Boolean.TRUE);
      ctx.timerService()
          .registerEventTimeTimer(ctx.timestamp() + stateRetentionDuration.toMillis());
      out.collect(value);
    }
  }
  ...
  public void open(Configuration config) {
    ValueStateDescriptor<Boolean> descriptor =
        new ValueStateDescriptor<>(stateName, TypeInformation.of(new TypeHint<Boolean>() {}));
