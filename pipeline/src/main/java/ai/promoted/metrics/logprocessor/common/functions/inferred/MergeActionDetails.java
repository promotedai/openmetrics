...
/**
 * Takes tiny Actions (from Inferred References) and fills in details using the stream of UnionEvent.
 *
 * TODO - maybe switch to regular interval join.  Research data lake first.
 * MergeImpressionDetails is still useful because we can consolidate DeliveryLog details into less state.
 */
public class MergeActionDetails extends AbstractMergeDetails {
    ...
    @Override
    protected void addIncompleteTimers(TinyEvent rhs, Context ctx, EnumSet<MissingEvent> missing, long timerTimestamp) throws Exception {
        for (MissingEvent missingEvent : missing) {
            switch (missingEvent) {
                case JOINED_IMPRESSION:
                    addIncompleteTimers(joinedImpressionMerger.impressionIdToIncompleteEventTimers, rhs.getImpressionId(), timerTimestamp);
                    break;
                case ACTION:
                    addIncompleteTimers(actionMerger.actionIdToIncompleteEventTimers, rhs.getActionId(), timerTimestamp);
                    break;
                default:
    ...
    final class JoinedImpressionMerger extends BaseMerger<JoinedEvent> {
        ...
        @VisibleForTesting
        MapState<String, JoinedEvent> idToJoinedImpression;
        ...
        public void open(RuntimeContext runtimeContext) throws Exception {
            idToJoinedImpression = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-joined-impression", String.class, JoinedEvent.class));
        ...
        @Override
        protected void tryProcessIncompleteEvents(JoinedEvent impression, boolean matchesDebugId, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger, Collector<JoinedEvent> out) throws Exception {
            tryProcessIncompleteEvents(impression.getIds().getImpressionId(), matchesDebugId, impressionIdToIncompleteEventTimers, errorLogger, out);
        }

        @Override
        protected String getDebugStateString(JoinedEvent impression) throws Exception {
            return getAllRelatedStateString(impression.getIds().getViewId(), impression.getIds().getRequestId(), impression.getIds().getImpressionId(), "");
        }
        ...
        protected String getDebugStateString(String id) throws Exception {
            ...
            return String.format("JoinedImpressionMerger{idToJoinedImpression[%s]=%s, impressionIdToIncompleteEventTimers[%s]=%s}", id, impression, id, incompleteEventTimers);
        }
        ...
        public void cleanup(long timestamp) throws Exception {
            cleanupDetailsState(idToJoinedImpression, timestamp, (JoinedEvent impression) -> impression.getTiming().getLogTimestamp(),
                    window, "JoinedImpression", debugIds::matchesImpressionId);
    ...
    final class ActionMerger extends BaseMerger<Action> {
        ...
        @VisibleForTesting
        MapState<String, Action> idToAction;
        ...
        public void open(RuntimeContext runtimeContext) throws Exception {
            idToAction = runtimeContext.getMapState(new MapStateDescriptor<>(
                    "id-to-action", String.class, Action.class));
            actionIdToIncompleteEventTimers = runtimeContext.getMapState(
                    // TODO(PRO-1683) - add caches back in.
                    new MapStateDescriptor<>("action-id-to-incomplete-event-timers", Types.STRING, Types.LIST(Types.LONG)));
        }
        ...
        @Override
        protected void tryProcessIncompleteEvents(Action action, boolean matchesDebugId, BiConsumer<OutputTag<MismatchError>, MismatchError> errorLogger, Collector<JoinedEvent> out) throws Exception {
            tryProcessIncompleteEvents(action.getActionId(), matchesDebugId, actionIdToIncompleteEventTimers, errorLogger, out);
        }

        @Override
        protected String getDebugStateString(Action action) throws Exception {
            return getAllRelatedStateString(action.getViewId(), action.getRequestId(), action.getImpressionId(), action.getActionId());
        }
        ...
        protected String getDebugStateString(String id) throws Exception {
            ...
            return String.format("ActionMerger{idToAction[%s]=%s, actionIdToIncompleteEventTimers[%s]=%s}", id, action, id, incompleteEventTimers);
        }
            ...
            if (!rhs.getActionId().isEmpty()) {
                ...
                if (action != null) {
                    ...
                    if (builder.getAction().hasCart()) {
                        ...
                        // The code tries to pick the most relevant CartItem.  If there are similar CartItems, the first one is picked.
                        Optional<CartContent> optionalCartContent = getMatchingCartContent(builder.getAction().getCart(), rhs);
        ...
        public void cleanup(long timestamp) throws Exception {
            cleanupDetailsState(idToAction, timestamp, (Action action) -> action.getTiming().getLogTimestamp(),
                    window, "Action", debugIds::matchesActionId);
