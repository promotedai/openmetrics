...
public abstract class ContentId<RHS extends GeneratedMessageV3> extends BaseInferred<RHS> {
   ...
   * @param rightContentsGetter Function to extract any content ids from the RHS. {@inheritDoc}
   */
  protected ContentId(
      String name,
  ...
  protected List<TinyEvent> inferJoin(String scopeKey, RHS rhs, Context ctx) throws Exception {
    ...
    // The following filter is just an optimization.  If a contentId is specified on the action,
    // narrow down all
    ...
    if (debugLogging) {
      LOGGER.info(
          "{} ContentId.inferJoin scopes: scopeKey={}\n scope={}\nrhs= {}",
          name,
          scopeKey,
          scope,
          rhs);
  ...
  private Predicate<TinyEvent> hasAnyMatchingContentId(Collection<String> rhsContentIds) {
    return (flat) -> {
      ...
      return rhsContentIds.stream()
          .anyMatch(
              rhsContentId ->
                  lhsContentIds.stream()
                      .anyMatch(
                          lhsContentId ->
                              !isBlank(lhsContentId)
                                  && !isBlank(rhsContentId)
                                  && Objects.equal(lhsContentId, rhsContentId)));
    };
  }
}
