...
final class ToTinyAction implements FlatMapFunction<Action, TinyEvent> {
    ...
    public void flatMap(Action action, Collector<TinyEvent> out) throws Exception {
        if (shouldSplitByCartContentIds(action)) {
            for (String contentId : getBaseAndCartContentIds(action)) {
                ...
                // TODO - if the cartContent.contentId matches the Action's primary contentId, we might be able
                // to keep the foreign keys.  It might get misused by clients.
                TinyEvent.Builder tinyAction = TinyFlatUtil.toTinyActionBuilder(action, contentId)
                        .clearImpressionId()
                        .clearInsertionId()
                        .clearRequestId()
                        .clearViewId();
                if (otherContentIdsConverter.hasKeys() && action.hasProperties()) {
                    otherContentIdsConverter.putFromProperties(tinyAction::putOtherContentIds, action.getProperties());
                }
                out.collect(tinyAction.build());
            }
        } else {
            TinyEvent.Builder tinyAction = TinyFlatUtil.toTinyActionBuilder(action, action.getContentId());
            if (otherContentIdsConverter.hasKeys() && action.hasProperties()) {
                otherContentIdsConverter.putFromProperties(tinyAction::putOtherContentIds, action.getProperties());
            }
            out.collect(tinyAction.build());
    ...
    private static Set<String> getBaseAndCartContentIds(Action action) {
        ...
        // TODO - decide if we should look at singleItemCart.  We don't expose it yet so we don't need to.
