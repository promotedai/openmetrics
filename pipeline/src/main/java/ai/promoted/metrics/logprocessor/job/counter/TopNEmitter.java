 ...
 *
 * Generally, this output should be used as the basis for a broadcast stream.  We need to keep the
...
public class TopNEmitter<T> extends RichFlatMapFunction<T, Tuple2<T, Boolean>> {
    ...
    public void open(Configuration config) {
        infos = getRuntimeContext().getMapState(
                new MapStateDescriptor<>("infos", typeInfo, TypeInformation.of(new TypeHint<HeapInfo<T>>(){})));
        minHeap = getRuntimeContext().getMapState(new MapStateDescriptor<>("min-heap", Types.LONG, typeInfo));
    ...
    private void siftDown(long i) throws Exception {
        ...
        long r = right(i);
        if (r < heapSize.value() && getCount(r) < getCount(smallest))
            smallest = r;
