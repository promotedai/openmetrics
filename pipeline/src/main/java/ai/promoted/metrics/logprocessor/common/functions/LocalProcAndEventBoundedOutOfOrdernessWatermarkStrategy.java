 ...
 */
public class LocalProcAndEventBoundedOutOfOrdernessWatermarkStrategy<T> implements WatermarkStrategy<T> {
    ...
    public LocalProcAndEventBoundedOutOfOrdernessWatermarkStrategy(
            Duration maxOutOfOrderness, Duration watermarkIdleness, Duration processingTimeTrailingDuration) {
    ...
    public WatermarkGenerator<T> createWatermarkGenerator(WatermarkGeneratorSupplier.Context var1) {
        return new LocalProcAndEventBoundedOutOfOrdernessWatermarkGenerator<T>(
                Clock.systemDefaultZone(), maxOutOfOrderness, watermarkIdleness, processingTimeTrailingDuration);
    }
}
