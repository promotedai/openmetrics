...
/**
 * A FlinkSegment that provides views as a source. It's a separate FlinkSegment to isolate the flags
 * to just jobs that use the deduplication source.
 */
public class RawViewSegment implements FlinkSegment {

  @CommandLine.Option(
      names = {"--keepFirstViewDuration"},
      defaultValue = "PT6H",
      description =
          "The duration to keep track of recent Views and AutoViews.  This is used to de-duplicate raw inputs.  Default=PT6H.  Java8 Duration parse format.")
  ...
  public DataStream<View> getDeduplicatedView(DataStream<View> view) {
    return baseFlinkJob.add(
        view.keyBy(KeyUtil.viewKeySelector)
            .process(
                new KeepFirstRow<>("view", keepFirstViewDuration), TypeInformation.of(View.class)),
        "keep-first-view");
  }
  ...
  public DataStream<AutoView> getDeduplicatedAutoView(DataStream<AutoView> autoView) {
    return baseFlinkJob.add(
        autoView
            .keyBy(KeyUtil.autoViewKeySelector)
            .process(
                new KeepFirstRow<>("autoview", keepFirstViewDuration),
