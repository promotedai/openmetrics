...
/**
 * A FlinkSegment that provides impressions as a source.  It's a separate FlinkSegment
...
public class RawImpressionSegment implements FlinkSegment {
    ...
    @CommandLine.Option(names = {"--keepFirstImpressionDuration"}, defaultValue = "P1D", description = "The duration to keep track of recent Impressions.  This is used to de-duplicate raw inputs.  Default=P1D to match flat_response_insertion join window.  Java8 Duration parse format.")
    ...
    public DataStream<Impression> getDeduplicatedImpression(DataStream<Impression> impressionDataStream) {
        return baseFlinkJob.add(
                impressionDataStream.keyBy(KeyUtil.impressionKeySelector)
                        .process(new KeepFirstRow<>("impression", keepFirstImpressionDuration),
