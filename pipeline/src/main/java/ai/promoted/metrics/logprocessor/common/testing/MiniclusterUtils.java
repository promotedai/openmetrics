...
public interface MiniclusterUtils {
    // Don't overly check for the job status to reduce load.  3ms was arbitrarily picked to trade off latency and load.
    long POLL_SLEEP_MS = 3;
    Logger LOGGER = LogManager.getLogger(MiniclusterUtils.class);

    static void waitForDone(ClusterClient<?> clusterClient, JobID jobId, Duration timeout) throws InterruptedException, ExecutionException {
        Instant start = Instant.now();

        // Accumulate job state for debugging.  Only record when the status changes.  Keep these two lists in sync.
        ...
        while (jobStatus == null || !jobStatus.isTerminalState()) {
            ...
            try {
                ...
                throw new RuntimeException("Job timed out, timeToStatus=" + fromJobStatusHistoryToString(statusHistory, timeHistory), e);
            }
        ...
        if (jobStatus != JobStatus.FINISHED) {
            SerializedThrowable throwable = clusterClient.requestJobResult(jobId).get().getSerializedThrowable().get();
            String jobException = getJobException(jobId);
            throw new RuntimeException("expected jobStatus=FINISHED, actual=" + jobStatus
                    + ", jobException=" + jobException
                    + ", timeToStatus=" + fromJobStatusHistoryToString(statusHistory, timeHistory)
    ...
    static String fromJobStatusHistoryToString(List<JobStatus> statuses, List<Long> times) {
        return Streams.zip(statuses.stream(), times.stream(), (status, time) -> status + "@" + time).collect(Collectors.joining(","));
    }
    ...
    static String toString(Throwable throwable) {
        return throwable + ",\nstackTrace=" + Joiner.on("\n").join(throwable.getStackTrace())
                + ",\ncause=" + (throwable.getCause() == null ? null : toString(throwable.getCause()));
    }
